...
export function getBusinessHoursConfig(tenant) {
  const pick = (raw) => {
    ...
    const days = Array.isArray(raw.days) ? raw.days : DEFAULT_BUSINESS_HOURS.days;
    return {
      start: raw.start ?? DEFAULT_BUSINESS_HOURS.start,
      end: raw.end ?? DEFAULT_BUSINESS_HOURS.end,
      days
    };
  };

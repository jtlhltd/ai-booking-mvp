...
export function getTenantTimezone(tenant, fallback = 'Europe/London') {
  return resolveTenantTimezone(tenant, fallback);
}

const DEFAULT_BUSINESS_HOURS = { start: 9, end: 17, days: [1, 2, 3, 4, 5] };

export function getBusinessHoursConfig(tenant) {
  const pick = (raw) => {
    if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return null;
    // Normalize days/start/end because these often arrive from JSON as strings.
    const daysRaw = Array.isArray(raw.days) ? raw.days : DEFAULT_BUSINESS_HOURS.days;
    const daysNorm = daysRaw
      .map((d) => {
    ...
    const days = daysNorm.length > 0 ? daysNorm : DEFAULT_BUSINESS_HOURS.days;
    return {
      start:
        raw.start == null
          ? DEFAULT_BUSINESS_HOURS.start
          : (typeof raw.start === 'string' ? Number(raw.start) : raw.start),
      end:
        raw.end == null
          ? DEFAULT_BUSINESS_HOURS.end
          : (typeof raw.end === 'string' ? Number(raw.end) : raw.end),
      days
    };
  };
  if (!tenant || typeof tenant !== 'object') return { ...DEFAULT_BUSINESS_HOURS };
  return pick(tenant.businessHours) || pick(tenant.whiteLabel?.businessHours) || { ...DEFAULT_BUSINESS_HOURS };
}
...
export function isBusinessHoursForTenant(
  ...
  } else if (options.forOutboundDial === true && allowOutboundWeekendCalls()) {
    for (const weekendDay of [0, 6]) {
      if (!days.includes(weekendDay)) days.push(weekendDay);
    }
  }

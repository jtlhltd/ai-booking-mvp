...
import { getCallsByTenant, getFullClient } from '../db.js';
...
export async function calculateROI(clientKey, days = 30, options = {}) {
  ...
  try {
    ...
    // Calculate bookings
    const bookings = periodCalls.filter(c => c.outcome === 'booked' || c.outcome === 'booking').length;
    
    // Calculate revenue (using client's avg deal value or default)
    const clientAvgDealValue = client.avgDealValue || avgDealValue;

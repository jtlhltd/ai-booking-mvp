...
function applyInHourJitter({ chosen, baseline, jitterKey, maxJitterSeconds }) {
  if (!chosen || !(chosen instanceof Date)) return chosen;
  if (!jitterKey) return chosen;
  if (chosen.getMinutes() !== 0 || chosen.getSeconds() !== 0 || chosen.getMilliseconds() !== 0) return chosen;
  ...
  const jitter = stableHashToInt(jitterKey) % (max + 1);

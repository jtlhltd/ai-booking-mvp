...
function collectCandidateSlots(tenant, baseline, { fallbackTz, maxDelayMs, preferredHourList = null, preferredWeekdays = null }) {
  ...
  const cfg = getBusinessHoursConfig(tenant);
  let tenantDays = Array.isArray(cfg.days) ? cfg.days : [1, 2, 3, 4, 5];
  if (!allowOutboundWeekendCalls()) {
    tenantDays = tenantDays.filter((d) => JS_WEEKDAYS_MON_FRI.includes(d));
    if (tenantDays.length === 0) tenantDays = [...JS_WEEKDAYS_MON_FRI];
  }
  ...
  for (let dayOffset = 0; dayOffset < 14; dayOffset++) {
    ...
    for (const hour of hourSource) {
      ...
      if (!isBusinessHoursForTenant(tenant, cand.toJSDate(), fallbackTz, { forOutboundDial: true })) continue;
...
export async function scheduleAtOptimalCallWindow(tenant, routing, baseline, options = {}) {
  ...
  const maxDelayMs = Number(process.env.OPTIMAL_CALL_MAX_DELAY_MS) > 0
    ...
    : 6 * 60 * 60 * 1000;
  const jitterParsed = Number(process.env.OPTIMAL_CALL_JITTER_MAX_SECONDS);
  const jitterMaxSeconds =
    process.env.OPTIMAL_CALL_JITTER_MAX_SECONDS !== undefined &&
    process.env.OPTIMAL_CALL_JITTER_MAX_SECONDS !== '' &&
    Number.isFinite(jitterParsed) &&
    jitterParsed >= 0
      ? jitterParsed
      : 55 * 60; // spread calls within the chosen hour; set env to 0 to disable
  ...
  if (disabled || !tenant) {
    return applyInHourJitter({
      chosen: clampOutboundDialToAllowedWindow(tenant, base, fallbackTz),
      baseline: base,
  ...
  if (!thompsonOff && options.clientKey != null) {
    ...
    if (th && th.getTime() !== base.getTime()) {
      const jittered = applyInHourJitter({
        chosen: clampOutboundDialToAllowedWindow(tenant, th, fallbackTz),
        baseline: base,
        jitterKey: options.jitterKey,
        maxJitterSeconds: jitterMaxSeconds,
        timeZone: jitterTz
  ...
  if (heuristic && heuristic.getTime() !== base.getTime()) {
    const jittered = applyInHourJitter({
      chosen: clampOutboundDialToAllowedWindow(tenant, heuristic, fallbackTz),
      baseline: base,
      jitterKey: options.jitterKey,
      maxJitterSeconds: jitterMaxSeconds,
      timeZone: jitterTz
  ...
  const resolved = heuristic || base;
  return applyInHourJitter({
    chosen: clampOutboundDialToAllowedWindow(tenant, resolved, fallbackTz),
    baseline: base,
    jitterKey: options.jitterKey,
    maxJitterSeconds: jitterMaxSeconds,
    timeZone: jitterTz

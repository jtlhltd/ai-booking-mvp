...
export async function callLeadInstantly({ clientKey, lead, client, allowOutsideBusinessHours = false }) {
  ...
  try {
    ...
    // Outbound A/B: voice, opening line, script via DB experiments (client.vapi.outboundAbExperiment)
    const outboundAbExperiment =
    ...
    if (outboundAbExperiment) {
      try {
        ...
        const selected = await selectABTestVariantForLead(clientKey, outboundAbExperiment, customerNumber);
        if (selected) {
          payload.metadata.abExperiment = outboundAbExperiment;
          payload.metadata.abVariant = selected.name;
          const { overrides, assistantIdOverride } = buildAssistantOverridesFromVariantConfig(selected.config);
          if (assistantIdOverride) {
            payload.assistantId = assistantIdOverride;
          }

...
export async function callLeadInstantly({ clientKey, lead, client, allowOutsideBusinessHours = false }) {
  ...
  try {
    ...
    const dialClaim = await claimOutboundDialSlotForToday(clientKey, rawPhone, tzDial);
    if (!dialClaim.ok) {
      console.log('[INSTANT CALL] Skipping — daily dial slot already used or held (local day)', {
        clientKey,
      ...
      return {
        ...
        details: 'At most one outbound attempt per phone per calendar day (tenant timezone).'
      };
    }

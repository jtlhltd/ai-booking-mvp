...
import { normalizeLeadDialContextEnvelope } from './lead-dial-context.js';
...
function shouldSkipLeadMessageOverride(text, clientKey) {
  const haystack = String(text || '').trim().toLowerCase();
  if (!haystack) return true;
  if (haystack.includes('client_key') || haystack.includes('tenant_key')) {
    return true;
  }
  const normalizedClientKey = String(clientKey || '').trim().toLowerCase();
  if (normalizedClientKey.length >= 6 && haystack.includes(normalizedClientKey)) {
    return true;
  }
  return false;
}

function buildLeadSystemMessageModelOverride(existingModel, systemMessage) {
  const current = existingModel && typeof existingModel === 'object' ? existingModel : {};
  return {
    ...current,
    provider: current.provider || 'openai',
    model: current.model || 'gpt-4o',
    temperature: current.temperature ?? 0.3,
    maxTokens: current.maxTokens ?? 500,
 ...
 */
export async function callLeadInstantly(params = {}) {
  const { clientKey } = params;
  return startSpan(
    { name: 'outbound.dial.instant', op: 'outbound.dial', attributes: { clientKey } },
    async () => callLeadInstantlyInner(params)
  );
}

async function callLeadInstantlyInner({
  clientKey,
  lead,
  ...
  try {
    ...
    const payload = {
      ...
      metadata: {
        ...
        leadId: lead.leadId != null ? lead.leadId : queueCallData?.leadId != null ? queueCallData.leadId : undefined
      }
    };
    ...
    // Multi-stage outbound sequence: merge per-stage overrides (tenant opt-in + kill switch in outbound-sequence.js)
    try {
      await startSpan(
        { name: 'outbound.sequence.resolve_overrides', op: 'outbound.sequence', attributes: { clientKey, stageId: queueCallData?.stageId } },
        async () => {
      const {
      ...
      if (
        !isOutboundSequenceGloballyDisabled() &&
        queueCallData &&
        queueCallData.stageId &&
        getValidatedOutboundSequence(client)
      ) {
        const { getLeadSequenceState } = await import('../db.js');
        const { resolveSequenceStageAssistantOverrides } = await import('./outbound-sequence-script-llm.js');
        const seqRow = await getLeadSequenceState(clientKey, customerNumber);
        const stId = String(queueCallData.stageId).trim();
        const seqStage = getStageById(client, stId);
        if (seqRow && seqStage) {
          const done = Array.isArray(seqRow.stagesCompleted) ? seqRow.stagesCompleted : [];
          const lastSnap = done.length ? done[done.length - 1] : null;
          const resolved = await resolveSequenceStageAssistantOverrides({
            stage: seqStage,
            lead: {
              ...lead,
              phone: lead?.phone || customerNumber,
              notes: lead?.notes || queueCallData?.leadNotes || '',
            },
            client,
            ctx: {
              stagesCompleted: done,
              currentStructured: {},
              isFinalStage: /** @type {any} */ (seqStage).isFinal === true,
              lastCallCompletedAt: lastSnap?.completedAt || null,
            },
            leadDialContextRaw: leadDialContext,
            correlationId,
          });
          const seqOverrides = resolved.overrides;
          const { mergeAssistantOverrides } = await import('./outbound-ab-variant.js');
          payload.assistantOverrides = mergeAssistantOverrides(payload.assistantOverrides || {}, seqOverrides);
          payload.metadata.stageId = stId;
          payload.metadata.sequenceAttempt = String((Number(seqRow.attemptsTotal) || 0) + 1);
          payload.metadata.sequenceScriptSource = resolved.scriptSource || 'static';
          if (resolved.llmModel) payload.metadata.sequenceLlmModel = String(resolved.llmModel);
          const soId = /** @type {any} */ (seqStage).structuredOutputId;
          if (soId && String(soId).trim()) {
            payload.metadata.structuredOutputId = String(soId).trim();
          }
        }
      }
        });
    } catch (e) {
      console.warn('[INSTANT CALL] outbound sequence overrides skipped:', e?.message || e);
    }
    ...
    try {
      await startSpan(
        { name: 'outbound.ab.select_variant', op: 'outbound.ab', attributes: { clientKey } },
        async () => {
      const {
    ...
    // Final per-lead overlay runs after sequence and A/B layers so CRM-imported
    // lead context wins last. PR4 kept this to variableValues only; PR5 extends
    // the same final overlay point to bounded first/system message text.
    const leadDialEnvelope = normalizeLeadDialContextEnvelope(leadDialContext);
    const leadDialVars = leadDialEnvelope.variableValues;
    if (Object.keys(leadDialVars).length > 0) {
      if (!payload.assistantOverrides) payload.assistantOverrides = {};
      const vv =
        payload.assistantOverrides.variableValues &&
        typeof payload.assistantOverrides.variableValues === 'object'
          ? payload.assistantOverrides.variableValues
          : {};
      payload.assistantOverrides.variableValues = {
        ...vv,
    ...
    if (leadDialEnvelope.firstMessage && !shouldSkipLeadMessageOverride(leadDialEnvelope.firstMessage, clientKey)) {
      if (!payload.assistantOverrides) payload.assistantOverrides = {};
      payload.assistantOverrides.firstMessage = leadDialEnvelope.firstMessage;
    } else if (leadDialEnvelope.firstMessage) {
      console.warn('[INSTANT CALL] Skipping lead firstMessage override with internal-key-like text');
    }
    if (leadDialEnvelope.systemMessage && !shouldSkipLeadMessageOverride(leadDialEnvelope.systemMessage, clientKey)) {
      if (!payload.assistantOverrides) payload.assistantOverrides = {};
      payload.assistantOverrides.model = buildLeadSystemMessageModelOverride(
        payload.assistantOverrides.model,
        leadDialEnvelope.systemMessage
      );
    } else if (leadDialEnvelope.systemMessage) {
      console.warn('[INSTANT CALL] Skipping lead systemMessage override with internal-key-like text');
    }
    ...
    try {
      result = await withCircuitBreaker(
        'vapi_call',
        async () => startSpan(
          { name: 'vapi.api.create_call', op: 'http.client', attributes: { clientKey, correlationId } },
          async () => {
          throwIfQueueDialAborted(signal);

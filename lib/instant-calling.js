...
//   - unknown-id releases: webhook landed on a different instance and
//     VAPI_CONCURRENCY_RELEASE_UNKNOWN=1 forced a slot release. This is the
//     ops footgun called out in docs/AUDIT_BACKLOG.md (P2).
let vapiSlotUnderflowCount = 0;
let vapiSlotUnknownReleaseCount = 0;
...
function _releaseOneSlot(reason = 'unknown') {
  ...
  if (reason === 'unknown_call_id_release') {
    vapiSlotUnknownReleaseCount += 1;
  }
...
export function getVapiConcurrencyState() {
  return {
    ...
    underflowCount: vapiSlotUnderflowCount,
    unknownReleaseCount: vapiSlotUnknownReleaseCount
  };
}

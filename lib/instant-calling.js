...
import {
  shouldUseDbSlotLeases,
  tryAcquireDbLeaseOnce,
  linkDbLeaseToCallId,
  releaseDbLeaseByCallId,
  releaseDbLeaseByLeaseId,
  getHeldDbLeaseCount
} from './vapi-slot-lease.js';
...
// Process-local diagnostics for the Vapi concurrency limiter. These are
// surfaced via getVapiConcurrencyState() and checked by lib/ops-invariants.js
// (intent: queue.concurrency-cap / billing.no-burst-dial). They count, since
// the process started:
//   - underflow attempts: _releaseOneSlot called with currentVapiCalls === 0
//     (i.e., release without a matching acquire — covered up by Math.max(0,…)
//     but never actually safe).
let vapiSlotUnderflowCount = 0;
...
function _releaseOneSlot(reason = 'unknown') {
  if (currentVapiCalls <= 0) {
    vapiSlotUnderflowCount += 1;
    console.warn('[VAPI CONCURRENCY] underflow_attempt — release without matching acquire', {
      reason,
      currentBeforeClamp: currentVapiCalls,
      underflowCount: vapiSlotUnderflowCount
    });
  }
...
/**
 * In-process slot limiter (Jest + single-node fallback when VAPI_DB_SLOT_LEASE=0).
 * @returns {Promise<void>}
 */
async function acquireVapiSlotMemory({ signal } = {}) {
...
/**
 * Acquire a global Vapi outbound slot. Returns `{ leaseId }` when DB leases are enabled
 * (Postgres/SQLite + not in Jest unless VAPI_DB_SLOT_LEASE=1); otherwise `{ leaseId: null }`.
 * @returns {Promise<{ leaseId: string | null }>}
 */
export async function acquireVapiSlot({ signal } = {}) {
  if (!shouldUseDbSlotLeases()) {
    await acquireVapiSlotMemory({ signal });
    return { leaseId: null };
  }
  const waitMs = Math.max(1000, Number(process.env.VAPI_SLOT_WAIT_MS || 120000) || 120000);
  const deadline = Date.now() + waitMs;
  while (Date.now() < deadline) {
    throwIfQueueDialAborted(signal);
    const r = await tryAcquireDbLeaseOnce();
    if (r.ok) return { leaseId: r.leaseId };
    await new Promise((res) => setTimeout(res, 50 + Math.floor(Math.random() * 150)));
  }
  throw new Error(`vapi_slot_wait_timeout_${waitMs}ms`);
}

export async function markVapiCallActive(callId, { ttlMs = 30 * 60 * 1000, phone = null, leaseId = null } = {}) {
  if (!callId) return;
  const normPhone = phone ? String(phone).trim() : null;
  if (leaseId && shouldUseDbSlotLeases()) {
    try {
      await linkDbLeaseToCallId(leaseId, callId);
    } catch (e) {
      console.warn('[VAPI LEASE] linkDbLeaseToCallId failed:', e?.message || e);
    }
  }
  ...
  const t = setTimeout(() => {
    if (activeVapiCalls.has(callId)) {
      ...
      if (shouldUseDbSlotLeases()) {
        releaseDbLeaseByCallId(callId).catch(() => {});
      } else {
  ...
  console.log('[VAPI CONCURRENCY] Call marked active:', {
    callId,
    phone: normPhone,
    current: shouldUseDbSlotLeases() ? getHeldDbLeaseCount() : currentVapiCalls,
    max: MAX_CONCURRENT_VAPI_CALLS
  });
}

export async function releaseVapiSlot({ callId, leaseId, reason = 'ended' } = {}) {
  if (shouldUseDbSlotLeases()) {
    if (callId && activeVapiCalls.has(callId)) {
      ...
      if (entry?.phone) activeVapiCallPhones.delete(entry.phone);
      activeVapiCalls.delete(callId);
      await releaseDbLeaseByCallId(callId);
      return;
    }
    if (callId) {
      await releaseDbLeaseByCallId(callId);
    ...
    if (leaseId) {
      await releaseDbLeaseByLeaseId(leaseId);
      return;
    }
    return;
  }
  ...
  if (callId) {
    console.log('[VAPI CONCURRENCY] Skip release — callId not active on this instance (memory mode):', {
      callId,
      reason,
...
export function getVapiConcurrencyState() {
  const dbLease = shouldUseDbSlotLeases();
  return {
    max: MAX_CONCURRENT_VAPI_CALLS,
    current: dbLease ? getHeldDbLeaseCount() : currentVapiCalls,
    queued: dbLease ? 0 : vapiSlotWaiters.length,
    activeIds: activeVapiCalls.size,
    activePhones: activeVapiCallPhones.size,
    underflowCount: dbLease ? 0 : vapiSlotUnderflowCount,
...
/**
 * Test-only: zero the diagnostic counters. The runtime should never call this.
 * Used by tests/canaries that share process state via dynamic import.
 */
export function _resetVapiConcurrencyDiagnosticsForTests() {
  vapiSlotUnderflowCount = 0;
}
...
export async function callLeadInstantly({
  ...
  try {
    ...
    const legacyExp = trimExp(vapi.outboundAbExperiment);
    ...
    throwIfQueueDialAborted(signal);
    const acq = await acquireVapiSlot({ signal });
    slotLeaseId = acq?.leaseId ?? null;
    slotAcquired = true;
    throwIfQueueDialAborted(signal);
    ...
    try {
      ...
      if (slotAcquired && !vapiSlotPersisted) {
        await releaseVapiSlot({ leaseId: slotLeaseId, reason: 'aborted_before_call_id' });
    ...
    await markVapiCallActive(result.id, {
      ttlMs: 30 * 60 * 1000,
      phone: customerNumber,
      leaseId: slotLeaseId
    });
 ...
 * IMPORTANT: This is the in-memory burst dialer. It is NOT the DB-backed
 * worker that drains the call_queue table on a cron — that lives in
 * server.js#processCallQueue and is the only thing scheduled-jobs.js wires
 * up. The two used to share a name; they were renamed apart in PR-9
 * (audit-backlog "dial.imports-distribute-not-burst") because the shared
 * name made it easy to accidentally re-introduce burst dialing. New
 * callers should use `dialLeadsNowBatch`. Importing `processCallQueue`
 * from this module is forbidden by scripts/check-policy.mjs unless
 ...
 */
export async function dialLeadsNowBatch(leads, client, options = {}) {
  const {

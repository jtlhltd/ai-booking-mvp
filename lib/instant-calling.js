...
export async function callLeadInstantly({ clientKey, lead, client }) {
  ...
  try {
    const customerNumber = (lead.phone && String(lead.phone).trim()) || '';
    if (!customerNumber) {
    ...
    if (!/^\+[1-9]\d{6,14}$/.test(customerNumber)) {
      console.warn('[INSTANT CALL] Lead phone may not be E.164:', customerNumber, '- VAPI may reject or fail with get customer');
    }

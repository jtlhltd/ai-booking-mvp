 ...
 */
export async function queueFirstSequenceStageForLead(params) {
  const { clientKey } = params;
  return startSpan(
    { name: 'outbound.sequence.queue_first_stage', op: 'outbound.sequence', attributes: { clientKey } },
    async () => queueFirstSequenceStageForLeadInner(params)
  );
}

async function queueFirstSequenceStageForLeadInner({
  clientKey,
  lead,
  client,
  actor = 'operator',

...
export function extractLogisticsFields(transcript) {
  ...
  } else if (/only\s+uk|uk\s+only|domestic\s+only/i.test(original)) {
  ...
  const weightDimsMatch = original.match(/(\d+(?:\.\d+)?)\s*kg/i);
  if (weightDimsMatch) {
    results.exampleShipment = `${weightDimsMatch[1]}kg`;
  }

  // Example shipment cost
  const costMatch = original.match(/£\s?(\d+(?:\.\d{2})?)/);
  if (costMatch) {
    results.exampleShipmentCost = `£${costMatch[1]}`;
  }
  ...
  // UK courier
  for (const [key, value] of Object.entries(courierMap)) {
    if (/\buk\b/i.test(original) && new RegExp(`\\b${key}\\b`, 'i').test(original)) {
  ...
  if (/excluding.*fuel|excl.*fuel|plus.*fuel|additional.*fuel/i.test(original)) {

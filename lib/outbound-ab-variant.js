 ...
 * @param {{ variantName?: string }} [options] When variantName is `control` and dimension is set, return no overrides so the live Vapi assistant wins for that slice.
 */
export function buildAssistantOverridesFromVariantConfig(config, dimension = null, options = {}) {
  const vn =
    options.variantName != null ? String(options.variantName).trim().toLowerCase() : '';
  const dim = dimension != null ? String(dimension).trim().toLowerCase() : '';
  if (vn === 'control' && (dim === 'voice' || dim === 'opening' || dim === 'script')) {
    return { overrides: {} };
  }

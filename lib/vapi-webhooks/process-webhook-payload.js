...
export async function processWebhookPayload(body, correlationId) {
    ...
    if (isEndOfCallReport && logisticsSheetId && (hasTranscript || effectiveHasStructuredData) && assistantMatches && !noUsefulOutcome) {
      ...
      try {
        ...
        // Persist per-call captured fields for UI attribution.
        // This lets the dashboard attribute "which call got which data" even when
        // lead_sequence_state / lead_handoff don't have callId-joined snapshots.
        try {
          if (callId && tenantKey) {
            const captured = {
              ...
              mainCouriers: sheetData.mainCouriers || '',
              internationalShipmentsPerWeek: sheetData.internationalShipmentsPerWeek || '',
              mainCountries: sheetData.mainCountries || '',
              exampleShipment: sheetData.exampleShipment || '',
              exampleShipmentCost: sheetData.exampleShipmentCost || '',
              ukShipmentsPerWeek: sheetData.ukShipmentsPerWeek || '',
              ukCourier: sheetData.ukCourier || '',
              standardRateUpToKg: sheetData.standardRateUpToKg || '',
              excludingFuelVat: sheetData.excludingFuelVat || '',
            ...
            const extractMeta = {
              ...
              source: sdHasMeaningfulValue ? 'analysis.structuredData' : (effectiveHasStructuredData ? 'structuredOutput' : 'transcript'),
              captured: compact,
            };

...
import { analyzeCall } from '../call-quality-analysis.js';
import messagingService from '../messaging-service.js';
import { sendOperatorAlert } from '../operator-alerts.js';
import { extractLogisticsFields } from '../logistics-extractor.js';
import { recordReceptionistTelemetry } from '../demo-telemetry.js';
import { storeCallContext } from '../call-context-cache.js';
...
import { pickCalleeBusinessNameForSheet } from './pick-callee-business-name.js';
import { updateCallTracking } from './update-call-tracking.js';
import {
  pickReceptionistName,
  handleBookingOutcome,
  handleInterestedProspect,
  handleFailedCall
} from './outcome-handlers.js';
import { startSpan } from '../sentry.js';
import { isLogisticsSheetWritesInCoreEnabled } from '../logistics-sheet-writes-in-core.js';
import { scheduleConsumerCallCompletedWebhook } from '../consumer-webhook-emitter.js';

export async function processWebhookPayload(body, correlationId) {
  const eventType = body?.message?.type || body?.type || 'unknown';
  const callId = body?.call?.id || body?.id || body?.callId || body?.message?.call?.id;

  return startSpan(
    {
      name: 'vapi.webhook.process',
      op: 'vapi.webhook',
      attributes: { correlationId, eventType, callId },
    },
    async () => processWebhookPayloadInner(body, correlationId)
  );
}

async function processWebhookPayloadInner(body, correlationId) {
    ...
    if (callId && looksEnded) {
      try {
        const { releaseVapiSlot } = await import('../instant-calling.js');
    ...
    const analysis = await startSpan(
      { name: 'vapi.webhook.analyze_call', op: 'call.quality', attributes: { correlationId, callId } },
      async () => analyzeCall({
        outcome,
        duration,
        transcript,
    ...
    await startSpan(
      { name: 'vapi.webhook.update_tracking', op: 'db', attributes: { correlationId, callId, clientKey: tenantKey } },
      async () => updateCallTracking({
        callId,
        tenantKey,
        leadPhone,
        status,
    ...
    if (toolCalls && toolCalls.length > 0) {
      ...
      const { handleVapiFunctionCall } = await import('../vapi-function-handlers.js');
      ...
      for (const toolCall of toolCalls) {
        try {
          ...
          // Legacy logistics tools — only when core sheet writes enabled (consumer apps own CRM by default)
          if (!isLogisticsSheetWritesInCoreEnabled(tenantKey)) {
            console.log('[VAPI WEBHOOK] Skipping legacy logistics tool call — LOGISTICS_SHEET_WRITES_IN_CORE disabled');
          } else if (toolCall.function.name === 'access_google_sheet') {
    ...
    const logisticsWritesInCore = isLogisticsSheetWritesInCoreEnabled(tenantKey);
    const logisticsSheetId = logisticsWritesInCore
      ? (tenant?.vapi?.logisticsSheetId || tenant?.gsheet_id || process.env.LOGISTICS_SHEET_ID || '1Tnll3FXtNEERYdGHTOh4VtAn90FyG6INUIU46ZbsP6g')
      : null;
    ...
    if (isEndOfCallReport && callId && hasTranscript && !hasStructuredData && process.env.VAPI_PRIVATE_KEY && logisticsWritesInCore) {
      try {
    ...
    // Persist per-call extracted fields for UI attribution, even when we skip sheet writes
    // due to noUsefulOutcome (voicemail, declined, etc).
    try {
      if (isEndOfCallReport && callId && tenantKey) {
        const analysisSdFlat =
          callObj?.analysis?.structuredData || body.analysis?.structuredData || msg?.analysis?.structuredData || {};
        const sd = (analysisSdFlat && typeof analysisSdFlat === 'object') ? analysisSdFlat : {};
        ...
        const captured = {
          ...
          callbackNeeded: sd.callbackNeeded ? 'TRUE' : '',
        };
        const compact = Object.fromEntries(Object.entries(captured).filter(([, v]) => String(v || '').trim() !== ''));
        const extractMeta = {
          extractedAt: new Date().toISOString(),
          source: Object.keys(compact).length ? 'analysis.structuredData' : (effectiveHasStructuredData ? 'structuredOutput' : 'none'),
          captured: compact,
        };
        const merged = (metadataForStore && typeof metadataForStore === 'object' && !Array.isArray(metadataForStore))
          ? { ...metadataForStore, structuredExtract: extractMeta }
          : { structuredExtract: extractMeta };
      ...
      console.warn('[LOGISTICS] per-call structuredExtract persist skipped:', metaErr?.message || metaErr);
    }

    if (isEndOfCallReport && tenantKey && leadPhone && callId) {
      try {
        const { handleOutboundSequenceEndOfCall } = await import('./outbound-sequence-webhook.js');
        const analysisSdFlat =
          callObj?.analysis?.structuredData || body.analysis?.structuredData || msg?.analysis?.structuredData || {};
        await handleOutboundSequenceEndOfCall({
          ...
          outcome,
          noUsefulOutcome,
          structuredSources: [effectiveStructuredOutput, analysisSdFlat]
        });
      } catch (seqErr) {
        console.warn(`[${correlationId}] [OUTBOUND SEQUENCE] handler error:`, seqErr?.message || seqErr);
      }
      scheduleConsumerCallCompletedWebhook({ clientKey: tenantKey, callId, leadPhone });
    }
    ...
    if (logisticsWritesInCore && isEndOfCallReport && logisticsSheetId && (hasTranscript || effectiveHasStructuredData) && assistantMatches && !noUsefulOutcome) {
            ...
            if (!hasAnyLogisticsInfo) {
              ...
              try {
                if (tenantKey && leadPhone) {
                  ...
                  const { collectOutboundAbExperimentNamesFromMetadata } = await import('../outbound-ab-live-pickup.js');
    ...
    } else if (logisticsWritesInCore && logisticsSheetId && callId && (recordingUrl || transcript) && !noUsefulOutcome) {
    ...
    if (outcome === 'booked' || body.booked === true) {
      ...
      const { scheduleFollowUps } = await import('../follow-up-sequences.js');

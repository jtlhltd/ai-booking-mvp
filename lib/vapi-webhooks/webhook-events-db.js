...
export async function tryInsertWebhookEvent({ provider, eventId, callId, eventType, correlationId, payload, headers }) {
  return startSpan(
    { name: 'vapi.webhook.idempotency', op: 'vapi.webhook', attributes: { eventId, callId, eventType, correlationId } },
    async () => tryInsertWebhookEventInner({ provider, eventId, callId, eventType, correlationId, payload, headers })
  );
}

async function tryInsertWebhookEventInner({ provider, eventId, callId, eventType, correlationId, payload, headers }) {
  if (!isPostgres()) {
...
export async function tryClaimExistingWebhookEvent({ provider, eventId }) {
  return startSpan(
    { name: 'vapi.webhook.idempotency', op: 'vapi.webhook', attributes: { eventId } },
    async () => tryClaimExistingWebhookEventInner({ provider, eventId })
  );
}

async function tryClaimExistingWebhookEventInner({ provider, eventId }) {

...
export async function runOutboundCallsForImportedLeads(params) {
  const { clientKey } = params;
  return startSpan(
    { name: 'lead.import.schedule_calls', op: 'lead.import', attributes: { clientKey } },
    async () => runOutboundCallsForImportedLeadsInner(params)
  );
}

async function runOutboundCallsForImportedLeadsInner({
  clientKey,
  inserted,
  isBusinessHours,
  getNextBusinessHour,
  ...
  const { getLeadDialContextRawFromLeadLike } = await import('./lead-dial-context.js');
  const { shouldLeadUseOutboundSequence } = await import('./outbound-sequence.js');
    ...
    try {
      ...
      await addToCallQueue({
        ...
        callData: {
          ...
          outboundDialMode: shouldLeadUseOutboundSequence(
            client,
            getLeadDialContextRawFromLeadLike(lead)
          ) ? 'sequence' : 'classic',
          leadId: lead.id,
          leadName: lead.name,

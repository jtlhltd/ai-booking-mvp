...
export async function checkRateLimit(identifier, endpoint, config = null) {
  ...
  // Count requests for this (identifier, endpoint) in current window.
  // Store keys are `${key}-${timestamp}`, so prefix-match is sufficient.
  const requests = Array.from(rateLimitStore.entries())
    .filter(([storeKey, timestamp]) => storeKey.startsWith(`${key}-`) && timestamp >= windowStart && timestamp <= now)
    .length;
  ...
  // Record this request
  // Ensure a unique key even when multiple requests occur in the same millisecond.
  rateLimitStore.set(`${key}-${now}-${rateLimitSeq++}`, now);

export const DASHBOARD_COHORT_FILTERS = Object.freeze(['all', 'classic', 'sequence', 'stopped', 'abandoned']);
...
export function getDashboardCohortMeta({
  ...
  const isStopped = source === OPERATOR_SEQUENCE_STOP_SOURCE;
  const isDismissedAbandoned = !!(isAbandoned && String(salvageDismissedAt || '').trim());
  const hasSequenceArtifact = !!hasSequenceState || isCompleted || isAbandoned || isStopped;

...
export function validateEnvironment() {
  ...
  if (String(process.env.NODE_ENV || '').toLowerCase() === 'production' && !process.env.ENCRYPTION_KEY) {
    console.warn(
      '[ENV VALIDATION] ENCRYPTION_KEY is not set. lib/security.js refuses production boot unless you set ENCRYPTION_UNUSED=1 when encryption features are unused.'
    );
  }

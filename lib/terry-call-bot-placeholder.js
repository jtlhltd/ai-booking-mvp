...
import { buildTerryArtifactPlan, TERRY_STRUCTURED_OUTPUT_ID } from './terry-structured-output.js';
import { getVapiPrivateKey } from './vapi-assistant-admin.js';
...
export function buildTerryPlaceholderAssistantPayload(options = {}) {
  ...
  const artifactPlan = buildTerryArtifactPlan();
...
/** Attach Terry structured output to an existing Vapi assistant (idempotent). */
export async function patchTerryAssistantStructuredOutput(assistantId) {
  const id = String(assistantId || '').trim();
  if (!id) throw new Error('assistantId is required');

  const artifactPlan = buildTerryArtifactPlan();
  if (!artifactPlan) {
    throw new Error('TERRY_STRUCTURED_OUTPUT_ID / VAPI_STRUCTURED_OUTPUT_ID is not configured');
  }
  ...
  const base =
    process.env.VAPI_ORIGIN && String(process.env.VAPI_ORIGIN).trim()
      ? String(process.env.VAPI_ORIGIN).replace(/\/+$/, '')
      : 'https://api.vapi.ai';

  const res = await fetch(`${base}/assistant/${encodeURIComponent(id)}`, {
    method: 'PATCH',
    headers: {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Failed to patch Terry structured output: ${text.slice(0, 240) || res.status}`);
  }

  return res.json();
}

export { TERRY_STRUCTURED_OUTPUT_ID };

 ...
 * Uses industry-specific templates for better conversion
 */
function generateSystemPrompt({ businessName, industry, primaryService, serviceArea, voiceGender }) {
  // Import industry templates
  const { customizeTemplate } = require('./industry-templates.js');
  
  try {
    // Get industry-specific template
    const template = customizeTemplate(industry, {
      businessName,
      primaryService,
      serviceArea,
      voiceGender
    ...
    // Return the optimized industry-specific prompt
    return template.systemPrompt;

  } catch (error) {
    console.warn('[AUTO-ONBOARD] Failed to load industry template, using generic prompt:', error);
...
export async function createClient({
  ...
  monthlyLeads,
  timezone,
  businessHours = '9am-5pm Mon-Fri', // Default value
  plan = 'custom'
}) {
  try {
    ...
    // 3. Generate custom AI prompt (using industry templates)
    ...
    console.log(`[AUTO-ONBOARD] Generated industry-specific AI prompt for ${industry}`);

    // 4. Set up client (no trial period)
    const trialEndsAt = null; // No trial period
    ...
    await query(`
      INSERT INTO api_keys (client_key, key_hash, key_name, created_at)
      VALUES ($1, $2, $3, $4)
    `, [clientKey, apiKeyHash, 'Primary API Key', new Date()]);

    console.log(`[AUTO-ONBOARD] ✅ API key created`);

    // 7. Store client metadata (create table if not exists)
    try {
      await query(`
      ...
      await query(`
        ...
        ON CONFLICT (client_key) DO UPDATE SET
          ...
          current_lead_source = EXCLUDED.current_lead_source,
          working_days = EXCLUDED.working_days,
          working_hours = EXCLUDED.working_hours,
          yearly_schedule = EXCLUDED.yearly_schedule,
      ...
      console.error('[AUTO-ONBOARD] Metadata table error:', metadataError);
 ...
 */
export async function sendWelcomeEmail({ clientKey, businessName, ownerEmail, apiKey, systemPrompt, businessSize, monthlyLeads, workingDays, workingHours, yearlySchedule }) {
  try {
        ...
        <div class="container">
          ...
          <div class="content">
            ...
            <div class="section">
              ...
              <p>Based on your signup, we've configured your system for:</p>
              <ul style="margin: 12px 0; line-height: 1.8;">
                ${businessSize ? `<li><strong>Business Size:</strong> ${businessSize}</li>` : ''}
                ${monthlyLeads ? `<li><strong>Lead Volume:</strong> ${monthlyLeads}</li>` : ''}
                ${workingDays ? `<li><strong>Working Days:</strong> ${workingDays}</li>` : ''}
                ${workingHours ? `<li><strong>Working Hours:</strong> ${workingHours}</li>` : ''}
                ${yearlySchedule ? `<li><strong>Yearly Schedule:</strong> ${yearlySchedule}</li>` : ''}
              </ul>
            </div>
            ...
            <div class="section">
              <h2>💳 Pricing</h2>
              <p>We'll discuss custom pricing based on your specific needs and lead volume.</p>
              <p>Most clients see 3-5x ROI within the first month.</p>
            </div>

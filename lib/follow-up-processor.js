...
import { isBusinessHoursForTenant, getNextBusinessOpenForTenant } from './business-hours.js';
    ...
    for (const followUp of dueFollowUps.rows) {
      try {
        ...
        if (followUp.retry_type === 'sms') {
          ...
          if (fuClient && !isBusinessHoursForTenant(fuClient)) {
            const next = getNextBusinessOpenForTenant(fuClient);
            await query(`
              UPDATE retry_queue 

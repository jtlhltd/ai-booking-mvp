...
import { phoneMatchKey, pgQueueLeadPhoneKeyExpr } from './lead-phone-key.js';

/**
 * Check if lead has opted out
 */
async function checkOptOut(clientKey, phone) {
  try {
    const { isOptedOut } = await import('./lead-deduplication.js');
    return await isOptedOut(clientKey, phone);
  } catch (error) {
  ...
  try {
    ...
    for (const followUp of dueFollowUps.rows) {
      try {
        ...
        if (await checkOptOut(clientKey, leadPhone)) {
        ...
        if (followUp.retry_type === 'sms') {
          ...
          const fuClient = await getFullClient(followUp.client_key);
          const fuTz = getTenantTimezone(fuClient, 'Europe/London');
          if (fuClient && !isBusinessHoursForTenant(fuClient, new Date(), fuTz, { forOutboundDial: true })) {
            const next = getNextBusinessOpenForTenant(fuClient, new Date(), fuTz, { forOutboundDial: true });
            await query(`
              UPDATE retry_queue 
            ...
            `, [next, followUp.id]);
            skipped++;
            console.log(`[FOLLOW-UP QUEUE] Call retry deferred — outside business hours (${followUp.lead_phone})`);
            continue;
          }
          const { hasOutboundWeekdayJourneyDialBlocked, updateRetryStatus } = await import('../db.js');
          const journeyBlk = await hasOutboundWeekdayJourneyDialBlocked(
            followUp.client_key,
            followUp.lead_phone,
            fuTz
          );
          if (journeyBlk.terminal) {
            await updateRetryStatus(followUp.id, 'cancelled');
            skipped++;
            console.log(
              `[FOLLOW-UP QUEUE] Call retry cancelled — outbound weekday journey complete (${followUp.lead_phone})`
            );
            continue;
          }
          if (journeyBlk.blocked) {
            const nextLocalDayStart = DateTime.now().setZone(fuTz).plus({ days: 1 }).startOf('day').toJSDate();
            const next = getNextBusinessOpenForTenant(
              fuClient || { booking: { timezone: fuTz }, timezone: fuTz },
              nextLocalDayStart,
              fuTz,
            ...
            skipped++;
            console.log(
              `[FOLLOW-UP QUEUE] Call retry deferred — weekday journey slot already used (${followUp.lead_phone})`
            );
            continue;
          }
          // For call retries, enqueue into call_queue instead of calling Vapi synchronously here.
          // This keeps the follow-up processor fast and lets the call_queue processor handle
          // concurrency, backoff (e.g. Vapi credits), and catch-up behavior.
          const clientKey = followUp.client_key;
          const leadPhone = followUp.lead_phone;
          try {
            ...
            if (!alreadyQueued) {
              ...
              await addToCallQueue({
                ...
                callType: 'vapi_call',
                callData: {
                  triggerType: 'follow_up_retry_queue',
                  retryQueueId: followUp.id,
                  retryReason: followUp.retry_reason || null,
                  retryAttempt: followUp.retry_attempt || null,
                  maxRetries: followUp.max_retries || null
                }
              });
              console.log(`[FOLLOW-UP QUEUE] ✅ Enqueued call retry into call_queue (${leadPhone})`);
            } else {
              console.log(`[FOLLOW-UP QUEUE] ℹ️ Call retry already queued (${leadPhone})`);
            }
          } catch (qErr) {
            // Let the normal retry handler below decide whether to reschedule or fail.
            throw qErr;
          }
        }
...
async function sendFollowUpSMS({ to, message, clientKey }) {
  try {
    ...
    if (await checkOptOut(clientKey, to)) {
 ...
 *
 * Exported for the retry-enqueues canary (tests/canaries/retry-enqueues.canary.test.js).
 * Production callers still go through processFollowUpQueue.
 */
export async function sendRetryCall({ clientKey, leadPhone, data }) {
  try {
    // Get client data
    const { getFullClient } = await import('../db.js');
    ...
    // Per-lead retry cap (intent: billing.max-retries-bounded). Prevents a
    // stuck retry loop or duplicated follow-up rows from generating an
    // unbounded number of Vapi calls for the same number. Tunable via
    // MAX_RETRIES_PER_LEAD (default 3) and MAX_RETRIES_PER_LEAD_WINDOW_HOURS
    // (default 24). Counts both pending and already-completed retry rows in
    // the window so a flapping lead cannot reset the cap by completing.
    const maxPerLead = Math.max(
      0,
      Math.min(1000, parseInt(String(process.env.MAX_RETRIES_PER_LEAD ?? '3'), 10) || 0)
    );
    const windowHours = Math.max(
      1,
      Math.min(24 * 30, parseInt(String(process.env.MAX_RETRIES_PER_LEAD_WINDOW_HOURS ?? '24'), 10) || 24)
    );
    if (maxPerLead > 0) {
      try {
        const phoneKey = phoneMatchKey(leadPhone) ?? '__nodigits__';
        const countRes = await query(
          ...
          `,
          [clientKey, phoneKey, windowHours]
        );
        const existing = countRes?.rows?.[0]?.n ?? 0;
        if (existing >= maxPerLead) {
          console.warn('[RETRY CALL] Skipping — max retries exceeded for this lead', {
            clientKey,
            leadPhone,
            existing,
            cap: maxPerLead,
          ...
          return {
            ...
            details: `Lead has ${existing} retry call(s) in the last ${windowHours}h (cap ${maxPerLead}).`
          };
        }
      } catch (capErr) {
        // Fail-open on the cap query: if we can't count, don't block the retry.
        console.warn('[RETRY CALL] Per-lead retry cap query failed (proceeding):', capErr?.message || capErr);
      }
    }

    // Do not dial synchronously here; enqueue and let the routing distribution + call_queue processor execute.
    ...
    const scheduledFor = await scheduleAtOptimalCallWindow(client, routing, new Date(), {
      fallbackTz: client?.booking?.timezone || client?.timezone || process.env.TZ || 'UTC',
      clientKey,
      jitterKey: `retry:${clientKey}:${lead.phone}`,
      banditArms
    });
    ...
    await addToCallQueue({
      ...
      callType: 'vapi_call',
      callData: {
        triggerType: 'follow_up_retry',
        leadId: data?.leadId || null,
        leadName: lead.name || data?.leadName || 'Prospect',
        leadService: lead.service || data?.service || 'consultation',
        retryQueueId: data?.retryQueueId || null
      }
    });

    console.log(`[RETRY CALL] ✅ Enqueued retry call to ${leadPhone}`, { scheduledFor: scheduledFor.toISOString() });
    return { ok: true, queued: true, scheduledFor: scheduledFor.toISOString() };

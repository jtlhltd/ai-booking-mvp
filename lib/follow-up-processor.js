    ...
    for (const followUp of dueFollowUps.rows) {
      try {
        ...
        if (followUp.retry_type === 'sms') {
          ...
          if (await hasOutboundCallAttemptToday(followUp.client_key, followUp.lead_phone, fuTz)) {
            ...
            skipped++;
            console.log(`[FOLLOW-UP QUEUE] Call retry deferred — daily dial limit (${followUp.lead_phone})`);
            continue;
          }

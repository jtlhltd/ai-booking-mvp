  ...
  no_answer: {
    ...
    steps: [
      ...
      },
      {
        ...
        channel: 'call',
        template: `Third attempt call - final follow-up`,
  ...
  interested_no_booking: {
    ...
    steps: [
      ...
      },
      {
        delay: 3 * 24 * 60 * 60 * 1000, // 3 days
        channel: 'sms',
        template: `Hi {name}, still interested in {service} at {businessName}? Reply YES to book or call {businessPhone}.`,
        nextAction: 'wait_for_response'
      },
      {
        delay: 7 * 24 * 60 * 60 * 1000, // 1 week
  ...
  try {
    ...
    for (const step of sequence.steps) {
      ...
      let message = step.template
        ...
        .replace(/{service}/g, 'appointment') // Can be customized per client
        .replace(/{businessPhone}/g, '555-1234') // Should pull from client config
        .replace(/{bookingLink}/g, 'https://booking.link') // Should generate per client
        .replace(/{benefit1}/g, 'Fast service')
        .replace(/{benefit2}/g, 'Professional team')
        .replace(/{benefit3}/g, 'Great results');

...
/**
 * Operator inbox for sample-ready emails.
 * Order: OUTBOUND_AB_SAMPLE_READY_EMAIL, YOUR_EMAIL (common on Render), then tenant vapi.
 */
export function resolveSampleReadyNotifyEmail(vapi) {
  const ab = process.env.OUTBOUND_AB_SAMPLE_READY_EMAIL;
  if (ab != null && String(ab).trim()) return String(ab).trim();

...
async function sendConfirmationSMS({ to, leadName, businessName, service, appointmentTime, location, businessPhone }) {
  try {
    const result = await messagingService.sendAppointmentConfirmationSMS({
      to,
      leadName,
      businessName,
      service,
    ...
    if (!result.success) {
      console.error('[SMS CONFIRMATION ERROR]', result.error);
    }
...
async function sendConfirmationEmail({ to, leadName, businessName, service, appointmentTime, location, businessPhone, appointmentId }) {
  try {
    const result = await messagingService.sendAppointmentConfirmationEmail({
      to,
      leadName,
      businessName,
      service,
...
export async function processReminderQueue() {
  try {
    // Get reminders that are due now (within next 5 minutes) - database-agnostic
    const { getRemindersDue } = await import('./database-health.js');
    const dueReminders = await getRemindersDue(5);
    ...
    for (const reminder of dueReminders.rows) {
      try {
        ...
        // Retry logic: increment retry_attempt and reschedule
        const retryAttempt = (reminder.retry_attempt || 0) + 1;
        ...
        if (retryAttempt < maxRetries) {
          ...
          await query(`
            UPDATE retry_queue 
            SET retry_attempt = $1, scheduled_for = $2, status = 'pending', updated_at = now()
            WHERE id = $3
          `, [retryAttempt, nextRetry, reminder.id]);
          
          console.log(`[REMINDER QUEUE] ⏳ Retry ${retryAttempt}/${maxRetries} scheduled for ${nextRetry.toISOString()}`);
        } else {
          // Max retries reached - move to dead letter queue
          const { moveToDLQ } = await import('./dead-letter-queue.js');
          await moveToDLQ({
            clientKey: reminder.client_key,
            originalTable: 'retry_queue',
            originalId: reminder.id,
            operationType: 'reminder',
            payload: data,
            errorHistory: [{ error: error.message, timestamp: new Date().toISOString() }],
            failureReason: `Reminder failed after ${maxRetries} attempts`,
          ...
          console.error(`[REMINDER QUEUE] ❌ Reminder ${reminder.id} failed after ${maxRetries} attempts - moved to DLQ`);
          
          // Send email alert for failed reminder
          if (process.env.YOUR_EMAIL) {
            try {
              await messagingService.sendEmail({
                to: process.env.YOUR_EMAIL,
                subject: `⚠️ Reminder Failed After ${maxRetries} Attempts`,
                body: `Reminder delivery failed\n\nReminder ID: ${reminder.id}\nPhone: ${reminder.lead_phone}\nAttempts: ${maxRetries}\nError: ${error.message}\nTime: ${new Date().toISOString()}\n\nMoved to Dead Letter Queue for manual review.`
              });
            } catch (emailError) {
              console.error('[REMINDER QUEUE] Failed to send alert email:', emailError.message);
            }
          }
        }
      }
...
async function sendReminderSMS({ to, message }) {
  try {
    ...
    if (!result.success) {
      console.error('[REMINDER SMS ERROR]', result.error);

 ...
 */
import { extractLeadDialContextFromImportLead } from './lead-dial-context.js';
import { startSpan } from './sentry.js';

export async function handleLeadsImport(req, res, deps) {
  return startSpan({ name: 'lead.import.upsert_batch', op: 'lead.import' }, async () => handleLeadsImportInner(req, res, deps));
}

async function handleLeadsImportInner(req, res, deps) {
  const {
    query,
    getClientFromHeader,
    isBusinessHours,
  ...
  try {
    ...
    for (const payload of leadsBatch) {
      ...
      const leadDialContext = extractLeadDialContextFromImportLead(payload);
      ...
      try {
        ...
        if (result?.row) inserted.push(result.row);
      } catch (insertError) {
        failedWrites += 1;

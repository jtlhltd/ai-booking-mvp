...
/**
 * Calculate dynamic lead score based on behavior
 * @param {Object} lead - Lead data
 * @param {Object} behavior - Behavior metrics
 * @returns {number} - Score 0-100
 */
export function calculateLeadScore(lead, behavior = {}) {
  let score = 50; // Start with neutral score
  
  // Add base score from lead data
  if (lead.rating) {
    score += (lead.rating - 50) * 0.2; // Convert 0-100 rating to ±10 points
  }
  
  // Behavior-based scoring
  if (behavior.callAnswered) {
    score += 20; // Big boost for answering calls
  }
  
  if (behavior.emailOpened) {
    score += 5; // Engagement through email
  }
  
  if (behavior.smsResponded) {
    score += 15; // SMS response shows high interest
  }
  
  if (behavior.websiteVisited) {
    score += 10; // Website visit shows interest
  }
  
  if (behavior.callDuration > 60) {
    score += 15; // Longer calls = more interest
  }
  
  // Time-based scoring
  const daysSinceContact = behavior.daysSinceContact || 365;
  if (daysSinceContact < 7) {
    score += 10; // Recent contact = warmer lead
  } else if (daysSinceContact > 30) {
    score -= 10; // Old contact = colder lead
  }
  
  // Interaction frequency
  if (behavior.callCount > 0) {
    score += Math.min(behavior.callCount * 5, 15); // More calls = more interest (cap at 15)
  }

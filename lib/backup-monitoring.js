...
export async function verifyBackupSystem() {
  try {
    ...
    if (!checks.databaseAccessible) {
      ...
      // Scenario 4: System has pending work but no activity - only warn if recently idle (not 30+ days)
      // After 30+ days idle, "pending work" is likely stale data; Render backups run regardless
      else if (checks.hasPendingWork && daysSinceActivity >= 3 && daysSinceActivity < 30) {
      ...
      else if (checks.hasPendingWork && daysSinceActivity >= 30) {
        checks.status = 'info';

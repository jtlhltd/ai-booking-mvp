...
export async function verifyBackupSystem() {
  try {
    const warningIdleDays = Number(process.env.BACKUP_WARNING_IDLE_DAYS || 7);
    const stalePendingDays = Number(process.env.BACKUP_STALE_PENDING_DAYS || 30);
    const checks = {
    ...
    // Check 2: Check if there are active clients configured (tenants use is_enabled, not active)
    try {
      const clientsResult = await query(`
        SELECT COUNT(*) as count 
        FROM tenants 
        WHERE is_enabled IS NOT FALSE
      `).catch(() => ({ rows: [{ count: '0' }] }));
    ...
    // Check 3: Pending work = recent actionable work (not stale data from months ago)
    try {
      const pendingWorkQueries = await Promise.allSettled([
        // Leads in active status that were touched recently (last 14 days)
        query(`SELECT COUNT(*) as count FROM leads WHERE status IN ('new', 'pending', 'follow_up') AND created_at > NOW() - INTERVAL '14 days'`).catch(() => ({ rows: [{ count: '0' }] })),
        // Future appointments (use start_iso; status is typically 'booked')
        query(`SELECT COUNT(*) as count FROM appointments WHERE start_iso > NOW() AND status IN ('booked', 'scheduled')`).catch(() => ({ rows: [{ count: '0' }] }))
      ]);
    ...
    if (!checks.databaseAccessible) {
      ...
      // Scenario 4: System has pending work but no activity
      // Warn only after a configurable idle threshold to avoid noisy alerts on short quiet periods.
      else if (checks.hasPendingWork && daysSinceActivity >= warningIdleDays && daysSinceActivity < stalePendingDays) {
      ...
      // Scenario 4b: Long-idle system with stale "pending" data - don't email warning
      else if (checks.hasPendingWork && daysSinceActivity >= stalePendingDays) {
        checks.status = 'info';
        checks.message = `System has been idle for ${daysSinceActivity.toFixed(1)} days (likely no recent leads/calls). Render Postgres backups continue automatically. Check Render Dashboard if you need to verify backups.`;
      }
...
export async function monitorBackups() {
  try {
    ...
    if (verification.status === 'warning' || verification.status === 'error') {
      ...
      const alertsSuppressed = process.env.ALERTS_SUPPRESSED === 'true' || process.env.MAINTENANCE_MODE === 'true';
      if (process.env.YOUR_EMAIL && !alertsSuppressed) {
        try {

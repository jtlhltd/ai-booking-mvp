...
// Initialize Redis on module load (skip in Jest — avoids lingering TCP handles in workers)
...
/** Close Redis client (tests / worker shutdown). In-memory cache singleton is left as-is. */
export async function disconnectRedisClient() {
  ...
  redisClient = null;
  if (instance) instance.useRedis = false;
}
...
export function cacheMiddleware(options = {}) {
  ...
  return async (req, res, next) => {
    ...
    // Try to get cached response (await since get() is async)
    try {
      ...
      console.warn('[CACHE] Cache get error, continuing:', cacheError.message);
    ...
    res.json = async (data) => {
      ...
      if (res.statusCode === 200) {
        try {
          ...
          console.warn('[CACHE] Cache set error:', cacheError.message);
        }
      }

 ...
 * @param {'voice'|'opening'|'script'} dimRaw
 */
function duplicateSingleToAbPair(one, dimRaw) {
  if (dimRaw === 'voice') {
    const v = one.voice;
    return [
      { name: 'control', voice: v },
      { name: 'variant_b', voice: v }
    ];
  }
  if (dimRaw === 'opening') {
    const m = one.firstMessage;
    return [
      { name: 'control', firstMessage: m },
      { name: 'variant_b', firstMessage: m }
    ];
  }
  const s = one.script;
  return [
    { name: 'control', script: s },
    { name: 'variant_b', script: s }
  ];
}
...
export function parseOutboundAbUploadSpec(jsonText, dimension) {
  ...
  if (variants.length === 1) {
    variants = duplicateSingleToAbPair(variants[0], dimRaw);
  }

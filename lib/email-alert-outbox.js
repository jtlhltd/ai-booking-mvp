...
export async function processEmailAlertOutbox({ limit = 10 } = {}) {
  if (operatorEmailAlertsDisabled()) {
    // Drain any queued items so disabling immediately stops any backlog from sending later.
    try {
      ...
      const n =
        drained && typeof drained.rowCount === 'number'
          ? drained.rowCount
          : Array.isArray(drained?.rows)
            ? drained.rows.length
            : null;
      if (n && n > 0) {
        console.log('[EMAIL OUTBOX] disabled: drained queued alerts', { cancelled: n });
      }
    } catch {
      // ignore drain failures while disabled
    }

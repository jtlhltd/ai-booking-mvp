...
export function extractObjections(transcript) {
  ...
  const objectionPatterns = {
    ...
    timing: [
      /\b(not now|too busy|bad time|not ready|maybe later)\b/gi,
      /\b(call\s+(?:me\s+)?back|call\s+back\s+later)\b/gi,
      /timing\s+is\s+not\s+right/gi,
      /timing.*not\s+right/gi,
      /not.*right\s+time/gi,
      /call\s+(?:me\s+)?back\s+(?:next|later|tomorrow|next week|next month)/gi
    ],
    ...
    trust: /\b(scam|spam|suspicious|trust|legitimate|real|verify|not sure about|unsure about|doubt)\b/gi,
  ...
  for (const [type, pattern] of Object.entries(objectionPatterns)) {
    // Handle timing as array of patterns
    if (type === 'timing' && Array.isArray(pattern)) {
      let timingMatch = false;
      for (const timingPattern of pattern) {
        const testPattern = new RegExp(timingPattern.source, timingPattern.flags);
        if (testPattern.test(transcript)) {
          timingMatch = true;
          // Don't flag as timing objection if it's in a positive booking context
          if (/book|appointment|schedule|meeting|confirm/i.test(transcript)) {
            timingMatch = false;
          }
...
export function extractKeyPhrases(transcript) {
  ...
  const phrasePatterns = [
    /\b(interested|interested in|want to know|tell me more|sounds interesting|curious about)\b/gi,
  ...
  for (const pattern of phrasePatterns) {
    // Reset regex state
    const testPattern = new RegExp(pattern.source, pattern.flags);
    const matches = transcript.match(testPattern) || [];
...
export function calculateQualityScore(callData) {
  ...
  if (transcript && transcript.length > 200) {
    ...
    score -= 1; // Very short transcript (might be incomplete or poor quality)
  }

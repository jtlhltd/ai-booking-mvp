...
/**
 * Postgres: indexed dial identity on `calls.lead_phone_match_key` (matches expression indexes).
 * Prefer over `pgQueueLeadPhoneKeyExpr` in hot paths — avoids per-row regexp in NOT EXISTS.
 */
export function pgCallsDialMatchKeyExpr(qualifiedMatchKeyCol = 'lead_phone_match_key') {
  return `COALESCE(${qualifiedMatchKeyCol}, '__nodigits__')`;
}

import { OPERATOR_SEQUENCE_STOP_SOURCE, SEQUENCE_ABANDONED_HANDOFF_SOURCE } from './dashboard-follow-up-filters.js';
import { startSpan } from './sentry.js';

export const MAX_SEQUENCE_OPS_AUDIT_ENTRIES = 32;
export { OPERATOR_SEQUENCE_STOP_SOURCE };
...
export async function stopOutboundSequenceForLead(params) {
  const { clientKey } = params;
  return startSpan(
    { name: 'outbound.sequence.stop', op: 'outbound.sequence', attributes: { clientKey } },
    async () => stopOutboundSequenceForLeadInner(params)
  );
}

async function stopOutboundSequenceForLeadInner({
  clientKey,
  leadPhone,
  actor = 'operator',
  ...
  const auditResult = await upsertLeadHandoffAudit({
    ...
    source: OPERATOR_SEQUENCE_STOP_SOURCE,
  ...
  return {
    ...
    status: 'abandoned',
    cancelledQueueRows,
    handoffSource: OPERATOR_SEQUENCE_STOP_SOURCE,
  };
}

export async function stopLeadsOutboundSequenceBulk(params) {
  const { clientKey } = params;
  return startSpan(
    { name: 'outbound.sequence.stop_bulk', op: 'outbound.sequence', attributes: { clientKey } },
    async () => stopLeadsOutboundSequenceBulkInner(params)
  );
}

async function stopLeadsOutboundSequenceBulkInner({
  clientKey,
  leadPhones,
  actor = 'operator',
  maxItems = 100,
  ...
  for (const raw of Array.isArray(leadPhones) ? leadPhones : []) {
    ...
    seen.add(phone);
    phones.push(phone);
    if (phones.length >= maxItems) break;
  }
  if (!clientKey || !phones.length) return { ok: false, error: 'no_lead_phones' };

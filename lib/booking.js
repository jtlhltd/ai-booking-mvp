...
export async function handleVapiBooking(payload) {
  const { clientKey, lead, service, slot } = normalizeFromVapi(payload);
  const tenant = await db.getTenant(clientKey);

  // Retry/idempotency guard: if we already booked this lead into this exact slot, do not duplicate side effects.
  const existing = await db.findExistingBooking({ tenantKey: clientKey, leadId: lead.id ?? null, slot });
  if (existing?.gcal_event_id) {
    return { ok: true, eventId: existing.gcal_event_id, deduped: true };
  }

...
async function executeWithRetry(fn, context = {}) {
  ...
  for (let attempt = 0; attempt <= RETRY_CONFIG.maxRetries; attempt++) {
    try {
      ...
      try {
        // Create timeout promise (must be cleared to avoid leaked handles)
        const timeoutPromise = new Promise((_, reject) => {
          timeoutId = setTimeout(() => reject(new Error('Request timeout')), RETRY_CONFIG.timeout);
        });
...
export class SalesforceIntegration {
  ...
  async syncLead(leadData) {
    try {
      ...
      const leadId = await this.findLead(phone, email);

...
// TODO(hygiene): this class declares searchProfessionalDirectories twice
// (lines ~1084 and ~2060). The second declaration silently overrides the
// first; consolidate to a single implementation in a follow-up.
...
export class RealDecisionMakerContactFinder {
    ...
    async scrapeWebsiteForTeam(website, business, industry, targetRole) {
        ...
        try {
            ...
            for (const teamPage of teamPages) {
                const teamContacts = await this.extractTeamContacts(teamPage, business, industry, targetRole);
    ...
    async extractTeamContacts(teamPageUrl, business, industry, targetRole) {
        const contacts = { primary: [], secondary: [], gatekeeper: [] };

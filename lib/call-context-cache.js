...
export function getMostRecentCallContext(tenantKey) {
  ...
  for (const [callId, context] of callContextCache.entries()) {
    ...
    console.log(`[CALL CONTEXT CACHE] Entry ${entryCount}:`, {
      ...
      timestamp: context.metadata?.timestamp || context.timestamp,
      age: Date.now() - (context.metadata?.timestamp || context.timestamp || 0) + 'ms'
    });
  }
  ...
  for (const [callId, context] of callContextCache.entries()) {
    const contextTenantKey = context.tenantKey || context.metadata?.tenantKey;
    ...
    if (matches) {
      matchCount++;
      const timestamp = context.timestamp || context.metadata?.timestamp || 0;

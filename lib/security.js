 ...
 * Verifies Twilio webhook signatures for security
 */
export function twilioWebhookVerification(req, res, next) {
  try {
    ...
    if (!twilioAuthToken) {
      console.warn('[TWILIO VERIFICATION] No auth token configured, skipping verification');
      return next();
    }
    
    // Skip verification if no signature provided
    if (!twilioSignature) {
      console.warn('[TWILIO VERIFICATION] No signature provided');
      return res.status(403).json({ error: 'Missing Twilio signature' });
    }
    
    // For now, we'll accept all requests with a signature
    // In production, you should implement proper signature verification
    // using Twilio's validation library
    
    console.log('[TWILIO VERIFICATION] Signature present, allowing request');
    return next();

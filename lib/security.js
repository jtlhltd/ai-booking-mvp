...
export { twilioWebhookVerification };

const isProdNode = String(process.env.NODE_ENV || '').toLowerCase() === 'production';
const encryptionExplicitlyUnused = ['1', 'true', 'yes'].includes(
  String(process.env.ENCRYPTION_UNUSED || '').trim().toLowerCase()
);
if (isProdNode && !process.env.ENCRYPTION_KEY && !encryptionExplicitlyUnused) {
  throw new Error(
    'Production requires ENCRYPTION_KEY (64 hex chars) for stable Encryption.* behaviour. ' +
      'If this deployment does not persist encrypted fields, set ENCRYPTION_UNUSED=1.'
  );
}
...
export class GDPRManager {
  ...
  async exportUserData(userId) {
    ...
    data.auditLogs = getAuditLogger().getLogs({ userId, limit: 1000 });
   ...
   * @param {Object} options - Options for retention
   * @returns {Object} Deletion summary
   */
  async applyDataRetention(days = 730, options = {}) {
    ...
    try {
      for (const table of tables) {
        try {
          ...
          if (dryRun) {
            ...
            const statusPlaceholders = preserveStatuses.map((_, i) => `$${i + 2}`).join(',');
            const countResult = await query(
              `SELECT COUNT(*) as count FROM ${table} 
               WHERE created_at < $1 
               AND (status IS NULL OR status NOT IN (${statusPlaceholders}))`,
            ...
            while (hasMore) {
              ...
              hasMore = (deleteResult.rowCount || 0) === batchSize;
            }
          }
      ...
      const totalDeleted = Object.values(summary.itemsDeleted).reduce((sum, count) => sum + (count || 0), 0);
      console.log(`[GDPR] Data retention applied: ${totalDeleted} items deleted across ${tables.length} table(s)`);
 ...
 */
export async function logAudit({ clientKey, action, details, ip, userAgent }) {
  try {

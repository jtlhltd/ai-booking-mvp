...
export async function getComprehensiveHealth(clientKey = null) {
  ...
  try {
    const { verifyBackupSystem } = await import('./backup-monitoring.js');
    ...
    health.services.backup = {
      ...
      daysSinceActivity: backupStatus.backupAge ? parseFloat((backupStatus.backupAge / 24).toFixed(1)) : null,
      databaseAccessible: backupStatus.databaseAccessible,
      recentActivity: backupStatus.recentActivity,
      hasActiveClients: backupStatus.hasActiveClients,
      totalClients: backupStatus.totalClients || 0,
      hasPendingWork: backupStatus.hasPendingWork
    };
    
    // Only mark as degraded/unhealthy for actual warnings/errors, not info messages
    if (backupStatus.status === 'warning' || backupStatus.status === 'error') {
      health.status = health.status === 'unhealthy' ? 'unhealthy' : 'degraded';
      health.alerts.push(`Backup system: ${backupStatus.message}`);
    }
  } catch (error) {
    health.services.backup = {
      status: 'error',
    ...
    console.error('[HEALTH] Failed to check backup status:', error);
  }

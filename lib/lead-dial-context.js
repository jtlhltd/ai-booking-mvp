...
export const CANONICAL_OUTBOUND_SEQUENCE_OPT_IN_KEY = 'outboundSequenceOptIn';

export const LEAD_OUTBOUND_SEQUENCE_OPT_IN_KEYS = Object.freeze([
  CANONICAL_OUTBOUND_SEQUENCE_OPT_IN_KEY,
  'sequenceOptIn',
  'multiCallOptIn',
  'multiStageOptIn'
]);
...
/**
 * Merge explicit multi-call enrollment into storable dial context.
 * @param {unknown} raw existing `lead_dial_context_json`
 * @param {{ enrolled: boolean }} options
 * @returns {Record<string, unknown>}
 */
export function mergeOutboundSequenceEnrollmentIntoDialContext(raw, { enrolled, actor, at }) {
  const envelope = normalizeLeadDialContextEnvelope(raw);
  const variableValues = { ...(envelope.variableValues || {}) };
  const ts = at ? String(at) : new Date().toISOString();
  const who = String(actor || 'operator').trim() || 'operator';

  variableValues[CANONICAL_OUTBOUND_SEQUENCE_OPT_IN_KEY] = !!enrolled;
  for (const key of LEAD_OUTBOUND_SEQUENCE_OPT_IN_KEYS) {
    if (key !== CANONICAL_OUTBOUND_SEQUENCE_OPT_IN_KEY) delete variableValues[key];
  }
  if (enrolled) {
    variableValues.sequenceEnrollmentAt = ts;
    variableValues.sequenceEnrollmentBy = who;
  } else {
    variableValues.sequenceUnenrolledAt = ts;
    variableValues.sequenceUnenrolledBy = who;
    const mode = variableValues.outboundDialMode;
    if (typeof mode === 'string' && mode.trim().toLowerCase() === 'sequence') {
      delete variableValues.outboundDialMode;
    }
  }

  const nextEnvelope = {
    variableValues,
    ...(envelope.firstMessage ? { firstMessage: envelope.firstMessage } : {}),
    ...(envelope.systemMessage ? { systemMessage: envelope.systemMessage } : {}),
  };
  return serializeLeadDialContextEnvelope(nextEnvelope, { preferEnvelope: true });
}
...
export function getLeadDialContextRawFromLeadLike(lead) {
  ...
  return (
    row.lead_dial_context_json ??
    row.leadDialContextJson ??
    row.leadDialContext ??
    row.dialContext ??
    row.customFields ??
    null

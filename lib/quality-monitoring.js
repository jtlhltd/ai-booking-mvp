...
import { getCallQualityMetrics, listClientSummaries, storeQualityAlert } from '../db.js';
import { enqueueEmailAlert } from './email-alert-outbox.js';
import { sendQualityAlert } from './email-alerts.js';

/** Hourly quality emails (plain outbox + HTML). Off unless explicitly enabled. */
function isQualityMonitoringEmailsEnabled() {
  const v = String(process.env.ENABLE_QUALITY_MONITORING_EMAILS || '').trim().toLowerCase();
...
export async function monitorCallQuality(clientKey) {
  ...
  try {
    ...
    // Queue an operator email (best-effort, deduped) when alerts exist.
    if (alerts.length > 0 && process.env.YOUR_EMAIL && isQualityMonitoringEmailsEnabled()) {
      try {
        const to = String(process.env.YOUR_EMAIL || '').trim();
        if (to) {
          const types = alerts.map((a) => a.type).filter(Boolean).slice(0, 6).join(',');
          await enqueueEmailAlert({
            clientKey,
            fingerprint: `quality_alert:${clientKey}:${types || 'alerts'}`,
            to,
            subject: `⚠️ Call quality alert(s) for ${clientKey}`,
            body: `Quality monitoring detected ${alerts.length} alert(s) for ${clientKey}.\n\nTypes: ${alerts
              .map((a) => a.type)
              .filter(Boolean)
              .join(', ') || 'unknown'}\n\nDashboard: ${process.env.BASE_URL || ''}/admin-hub.html\n`,
            meta: { alerts, metrics: { total_calls: totalCalls, ...rates } }
          });
        }
      } catch (e) {
        console.error('[QUALITY MONITOR] Failed to enqueue email alert:', e?.message || e);
      }
    }
...
export async function monitorAllClients() {
  ...
  try {
    ...
    const activeClients = summaries.filter(c => c.isEnabled !== false);
    ...
    for (const s of activeClients) {
      const result = await monitorCallQuality(s.clientKey);
      results.push(result);
      
      // If there are alerts, store them
      if (result.alerts && result.alerts.length > 0) {
        const clientForAlerts = {
          client_key: s.clientKey,
          displayName: s.displayName,
          contact: { email: s.email || null }
        };
        await handleQualityAlerts(clientForAlerts, result.alerts, result.metrics);
      }
    }
...
async function handleQualityAlerts(client, alerts, metrics) {
  try {
    ...
    if (importantAlerts.length > 0 && isQualityMonitoringEmailsEnabled()) {

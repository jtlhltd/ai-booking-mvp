  ...
  try {
    ...
    for (const retry of dueRetries.rows) {
      try {
        ...
        const isAbsoluteUrl = /^https?:\/\//i.test(String(webhookUrl || ''));
        const fullUrl = isAbsoluteUrl
          ? webhookUrl
          : `${process.env.BASE_URL || 'http://localhost:3000'}${webhookUrl}`;
        ...
        if (response.ok) {
          ...
          if (retry.retry_attempt >= retry.max_retries) {
            ...
            await moveToDLQ({
              clientKey: retry.client_key,
              originalTable: 'retry_queue',
              originalId: retry.id,
              operationType: `webhook_${webhookType}`,
              payload: retryData,
              errorHistory: [{ error: `${response.status} ${response.statusText}`, timestamp: new Date().toISOString() }],
              failureReason: `Webhook failed after ${retry.max_retries} retry attempts`,
              retryCount: retry.retry_attempt,
            ...
            failureCount++;
            console.error(`[WEBHOOK RETRY] ❌ Failed after max retries: ${webhookType} (attempt ${retry.retry_attempt}) - moved to DLQ`);

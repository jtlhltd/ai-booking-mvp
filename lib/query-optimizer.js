...
export async function optimizedQuery(sql, params = [], options = {}) {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    let timeoutHandle;
    try {
      // IMPORTANT: clear the timeout when the query settles, otherwise every
      // call leaks an active timer until `timeout` ms have elapsed. Under
      // load this used to fire thousands of stray rejections and trip the
      // Jest "worker failed to exit gracefully" warning.
      const timeoutPromise = new Promise((_, reject) => {
        timeoutHandle = setTimeout(() => reject(new Error(`Query timeout after ${timeout}ms`)), timeout);
      ...
      if (attempt < retries) {
        await new Promise((resolve) => {
          const t = setTimeout(resolve, retryDelay * (attempt + 1));

...
import { REQUEST_QUEUE_HANDLED_CALL_TYPES } from './request-queue-handled-types.js';
import { startSpan } from './sentry.js';

export { REQUEST_QUEUE_HANDLED_CALL_TYPES } from './request-queue-handled-types.js';
...
export async function enqueueRequest({
  ...
  try {
    ...
    if (!REQUEST_QUEUE_HANDLED_CALL_TYPES.includes(requestType)) {
      return {
        success: false,
        error: `requestType must be one of: ${REQUEST_QUEUE_HANDLED_CALL_TYPES.join(', ')}`
      };
    }
    if (!scheduledFor) {
...
export async function processQueue(options = {}) {
  ...
  try {
    // Atomically claim work so multiple workers/processes don't double-process the same row.
    // Postgres: use SKIP LOCKED to safely run multiple workers.
    // SQLite/JSON: best-effort claim per-row via conditional UPDATE ... RETURNING.
    let claimedRows = [];
    if (dbType === 'postgres') {
    ...
    console.log(`[REQUEST QUEUE] Processing ${claimedRows.length} queued requests`);
    
    let processed = 0;
    let failed = 0;
    
    // Process in batches of maxConcurrent
    for (let i = 0; i < claimedRows.length; i += maxConcurrent) {
      const batch = claimedRows.slice(i, i + maxConcurrent);
      
      const batchPromises = batch.map(async (item) => {
        try {
          // For non-Postgres, "claim" per row (prevents double-processing across multiple workers).
          if (dbType !== 'postgres') {
            ...
            const didClaim = Array.isArray(claim?.rows) ? claim.rows.length > 0 : !!claim?.changes;
            if (!didClaim) {
              return { success: true, id: item.id, skipped: true };
            }
          }
    ...
    return { processed, failed, queued: claimedRows.length };
  } catch (error) {
...
async function processRequest(requestType, payload, clientKey) {
  switch (requestType) {
    ...
    case 'lead_import':
      await startSpan(
        { name: 'lead.import.async_queue_item', op: 'lead.import', attributes: { clientKey } },
        async () => {

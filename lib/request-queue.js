      ...
      const batchPromises = batch.map(async (item) => {
        try {
          ...
          if (String(error.message || '').includes('outside_business_hours')) {
            ...
            const { getNextBusinessOpenForTenant } = await import('./business-hours.js');
            const rqClient = await getFullClient(item.client_key);
            const rqTz = process.env.TIMEZONE || process.env.TZ || 'Europe/London';
            const next = rqClient
...
async function processRequest(requestType, payload, clientKey) {
  switch (requestType) {
    case 'vapi_call':
      ...
      const { getFullClient } = await import('../db.js');
      const { isBusinessHoursForTenant, getTenantTimezone } = await import('./business-hours.js');
      
      // Get client data
      const client = await getFullClient(clientKey);

      const vTz = getTenantTimezone(client, process.env.TIMEZONE || 'Europe/London');
      if (client && !isBusinessHoursForTenant(client, new Date(), vTz, { forOutboundDial: true })) {
        console.log('[REQUEST QUEUE] Skipping vapi_call — outside business hours', clientKey);

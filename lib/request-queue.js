      ...
      const batchPromises = batch.map(async (item) => {
        try {
          ...
          if (String(error.message || '').includes('outside_business_hours')) {
            ...
            const next = rqClient ? getNextBusinessOpenForTenant(rqClient) : new Date(Date.now() + 60 * 60 * 1000);
            await query(`
              UPDATE call_queue
              SET status = 'pending',
                  scheduled_for = $1,
...
async function processRequest(requestType, payload, clientKey) {
  switch (requestType) {
    case 'vapi_call':
      ...
      const { getFullClient } = await import('../db.js');
      const { isBusinessHoursForTenant } = await import('./business-hours.js');
      
      // Get client data
      const client = await getFullClient(clientKey);

      if (client && !isBusinessHoursForTenant(client)) {

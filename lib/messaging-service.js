...
class MessagingService {
  ...
  initialize() {
    try {
      ...
      if (process.env.TWILIO_ACCOUNT_SID && process.env.TWILIO_AUTH_TOKEN) {
        ...
        // In test/dev, missing provider credentials is common and shouldn't spam logs.
        if (!isTestEnv()) console.warn('⚠️ Twilio credentials not found - SMS disabled');
      }
      ...
      if (process.env.EMAIL_SERVICE && process.env.EMAIL_USER && process.env.EMAIL_PASS) {
        ...
        if (!isTestEnv()) console.warn('⚠️ Email credentials not found - Email disabled');
      }
  ...
  async sendSMS({ to, body, from = null, messagingServiceSid = null }) {
    if (!this.twilioClient) {
      if (!isTestEnv()) console.error('[SMS] Twilio not configured');
    ...
    try {
      ...
      if (messagingServiceSid) {
        ...
        if (!isTestEnv()) console.error('[SMS] No from number or messaging service SID configured');
   ...
   * @returns {Promise<Object>} - Send result
   */
  async sendEmail({ to, subject, body, html = null, from = null, attachments = null }) {

...
function buildEmailHtml({ clientKey, dimensionLabel, experimentName, summary, minPerVariant, dashboardHint }) {
  const rows = summary.variants
    .map((v) => {
      ...
      const fuRate =
        v.followUpRatePct != null && Number.isFinite(Number(v.followUpRatePct))
          ? `${Number(v.followUpRatePct).toFixed(1)}%`
          : '—';
      const lp =
      ...
      return `<tr><td>${escapeHtml(v.variantName)}</td><td style="text-align:right">${escapeHtml(
        ...
        String(fu)
      )}</td><td style="text-align:right">${escapeHtml(fuRate)}</td></tr>`;
    })
    .join('');
  return `
  ...
  <thead><tr><th>Variant</th><th>Live pickups</th><th>Assigned leads</th><th>Follow-up dials</th><th>% w/ follow-up</th></tr></thead>
  <tbody>${rows}</tbody>
  </table>

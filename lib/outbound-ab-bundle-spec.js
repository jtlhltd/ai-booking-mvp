...
export function stringsToMappedVariants(dimension, values) {
  ...
  return labels.map((name, i) => {
    ...
    if (dim === 'voice') return { name, config: { voice: val } };
...
function takeStringArray(root, keys) {
  ...
  for (const k of keys) {
    if (!Array.isArray(root[k]) || root[k].length < 2) continue;
 ...
 * @returns {{ voices: string[], openings: string[], scripts: string[] }}
 */
export function parseOutboundAbBundleSpec(jsonText) {
  ...
  if (!voices) {
    throw new Error('Missing "voices" (or voiceIds): need an array of at least 2 non-empty voice IDs');
  }
  if (!openings) {
    throw new Error('Missing "openings" (or openingLines): need an array of at least 2 non-empty lines');
  }
  if (!scripts) {
    throw new Error('Missing "scripts": need an array of at least 2 non-empty script strings');
  }

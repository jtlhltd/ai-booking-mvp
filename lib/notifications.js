...
<body>
  ...
  try {
    const { default: MessagingService } = await import('./messaging-service.js');
    ...
    const result = await messagingService.sendEmail({
      ...
      html: emailContent,
      body: `Daily Summary for ${clientName}: ${appointmentsBooked} appointments, ${callsMade} calls, ${conversionRate}% conversion`
    });
    
    if (result.success) {
      console.log(`[DAILY SUMMARY] ✅ Sent to ${clientEmail}`);
    } else {
      console.error(`[DAILY SUMMARY] ❌ Failed to send to ${clientEmail}:`, result.error);
    }
    
    return result;
  } catch (error) {
    console.error('[DAILY SUMMARY] Email sending error:', error);
    return {
      success: false,
      error: error.message,
      to: clientEmail,

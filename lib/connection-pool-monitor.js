...
export async function checkPoolHealth() {
  try {
    ...
    if (pool.options && pool.options.max) {
      ...
      // Fallback: if we can't get max, use a reasonable default (15) instead of total
      // This prevents false alerts when pool is working normally
      poolMax = 15;
      console.warn('[POOL MONITOR] Could not read pool.max, using default of 15');
    }
    ...
    if (utilizationOfMax >= threshold && total > 0) {
      ...
      console.log(`[POOL MONITOR] Alert triggered: ${active}/${poolMax} active (${utilizationOfMax.toFixed(1)}% of max), threshold: ${threshold}%`);
      alerts.push({
        level: 'warning',
      ...
      // Log when we're NOT alerting (for debugging)
      if (active > 0) {
        console.log(`[POOL MONITOR] Pool healthy: ${active}/${poolMax} active (${utilizationOfMax.toFixed(1)}% of max), threshold: ${threshold}%`);
      }
    }

...
export async function sendQualityAlert(client, alerts, metrics) {
  if (!isQualityMonitoringEmailsEnabled()) {
    console.log('[EMAIL ALERT] Quality monitoring emails disabled (set ENABLE_QUALITY_MONITORING_EMAILS=true to enable)');
    return false;
  }
  ...
  const overrideTo = process.env.YOUR_EMAIL != null && String(process.env.YOUR_EMAIL).trim() !== ''
    ? String(process.env.YOUR_EMAIL).trim()
    : '';
  const toEmail = overrideTo || (contact?.email != null ? String(contact.email).trim() : '');
  ...
  const htmlBody = `
    ...
    <body>
      <div class="container">
        ...
        <div class="footer">
          <p>AI Booking System - Quality Monitoring System</p>
  ...
  try {
    ...
    console.log(`[EMAIL ALERT] ✅ Sent to ${toEmail} (${alerts.length} alerts, severity: ${severity})`);
    return true;
  } catch (error) {
...
export async function sendWeeklySummary(client, weeklyStats) {
  ...
  const overrideTo = process.env.YOUR_EMAIL != null && String(process.env.YOUR_EMAIL).trim() !== ''
    ? String(process.env.YOUR_EMAIL).trim()
    : '';
  const toEmail = overrideTo || (client?.contact?.email != null ? String(client.contact.email).trim() : '');
      ...
      <div style="max-width: 600px; margin: 0 auto; background: white;">
        ...
        <div style="background: #f8f9fa; padding: 30px; text-align: center; border-top: 1px solid #e9ecef;">
          ...
          <p style="color: #999; font-size: 12px; margin: 10px 0 0 0;">
            AI Booking System - Weekly Performance Reports
          </p>
        </div>
      </div>
    </body>
  ...
  try {
    await transporter.sendMail(mailOptions);
    console.log(`[WEEKLY SUMMARY] ✅ Sent to ${toEmail}`);
    return true;
  } catch (error) {

...
import { pgCallsDialMatchKeyExpr, pgQueueLeadPhoneKeyExpr, phoneMatchKey } from './lead-phone-key.js';
...
async function processSheetPatchRetry(retry) {
  if (!isLogisticsSheetWritesInCoreEnabled(retry?.client_key)) {
    console.log('[SHEET PATCH RETRY] Skipped — LOGISTICS_SHEET_WRITES_IN_CORE disabled');
    return;
  }
  const retryData = (() => {
  ...
  try {
    ...
    try {
      ...
      if (pendingTotal <= catchupMinPending) {
        ...
        const lookbackDays = Math.max(1, Math.min(730, parseInt(process.env.FAILED_Q_CATCHUP_LOOKBACK_DAYS || '90', 10) || 90));

        const dialMatchKey = pgCallsDialMatchKeyExpr('lead_phone_match_key');
        const { rows: clientsWithBacklog } = await query(
          `
            SELECT client_key, COUNT(DISTINCT ${dialMatchKey})::int AS n
            FROM calls
        ...
        const cDialKey = pgCallsDialMatchKeyExpr('c.lead_phone_match_key');
        const okDialKey = pgCallsDialMatchKeyExpr('ok.lead_phone_match_key');
        ...
        for (const row of clientsWithBacklog) {
          ...
          const { rows: phones } = await query(
            `
              WITH candidates AS (
                SELECT
                  (ARRAY_AGG(c.lead_phone ORDER BY c.created_at DESC))[1] AS lead_phone,
                  MAX(c.created_at) AS last_failed
                FROM calls c

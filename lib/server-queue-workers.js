...
export async function processVapiCallFromQueue(call) {
  try {
    ...
    const leadDialContext = normalizeLeadDialContext(existingLead?.lead_dial_context_json);
        ...
        for (const lead of newLeads.rows) {
          try {
            ...
            try {
              ...
              const { getFirstStage, getValidatedOutboundSequence, isOutboundSequenceGloballyDisabled } =
                await import('./outbound-sequence.js');
              if (!isOutboundSequenceGloballyDisabled() && getValidatedOutboundSequence(client)) {

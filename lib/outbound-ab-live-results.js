...
function mapVariantRows(summary, minN) {
  ...
  return summary.variants.map((v) => {
    ...
    const med =
      v.medianTalkSeconds != null && Number.isFinite(Number(v.medianTalkSeconds))
        ? Number(v.medianTalkSeconds)
        : null;
    const durN =
      typeof v.livePickupDurationCount === 'number'
        ? v.livePickupDurationCount
        : parseInt(v.livePickupDurationCount, 10) || 0;
    const avgComp =
      v.avgCompletenessScore != null && Number.isFinite(Number(v.avgCompletenessScore))
        ? Number(v.avgCompletenessScore)
        : null;
    const compN =
      typeof v.convertedCompletenessCount === 'number'
        ? v.convertedCompletenessCount
        : parseInt(v.convertedCompletenessCount, 10) || 0;
    return {
      ...
      livePickupDurationCount: durN,
      medianTalkSeconds: med,
      convertedCompletenessCount: compN,
      avgCompletenessScore: avgComp,

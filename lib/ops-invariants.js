...
async function checkOneClient({
  ...
  rows.sequenceStaleActive = await query(
    `
      SELECT COUNT(*)::int AS n
  ...
  rows.sequenceDupActiveQueue = await query(
    `
      SELECT COUNT(*)::int AS n
      FROM (
        SELECT lead_phone
  ...
  rows.sequenceAttemptsInStageOverMax = { rows: [{ n: 0 }] };
  rows.sequenceAttemptsTotalOverCap = { rows: [{ n: 0 }] };
  rows.sequenceCalendarDurationExceeded = { rows: [{ n: 0 }] };
  if (isPostgres) {
    rows.sequenceAttemptsInStageOverMax = await query(
      `
        SELECT COUNT(*)::int AS n
        FROM lead_sequence_state lss
        INNER JOIN tenants t ON t.client_key = lss.client_key
        CROSS JOIN LATERAL jsonb_array_elements(COALESCE(t.outbound_sequence_json->'stages', '[]'::jsonb)) AS elem
        WHERE lss.client_key = $1
          AND lss.status = 'active'
          AND COALESCE((t.outbound_sequence_json->>'enabled')::boolean, false) = true
          AND (elem->>'id') = lss.current_stage_id
          AND (elem->>'maxAttemptsInStage') ~ '^[0-9]+$'
          AND lss.attempts_in_stage > (elem->>'maxAttemptsInStage')::int
      `,
      [ck]
    );
    rows.sequenceAttemptsTotalOverCap = await query(
      `
        SELECT COUNT(*)::int AS n
        FROM lead_sequence_state lss
        INNER JOIN tenants t ON t.client_key = lss.client_key
        WHERE lss.client_key = $1
          AND lss.status = 'active'
          AND COALESCE((t.outbound_sequence_json->>'enabled')::boolean, false) = true
          AND (t.outbound_sequence_json->>'maxTotalDialsPerLead') ~ '^[0-9]+$'
          AND lss.attempts_total > (t.outbound_sequence_json->>'maxTotalDialsPerLead')::int
      `,
      [ck]
    );
    rows.sequenceCalendarDurationExceeded = await query(
      `
        SELECT COUNT(*)::int AS n
        FROM lead_sequence_state lss
        INNER JOIN tenants t ON t.client_key = lss.client_key
        WHERE lss.client_key = $1
          AND lss.status = 'active'
          AND COALESCE((t.outbound_sequence_json->>'enabled')::boolean, false) = true
          AND (t.outbound_sequence_json->>'maxSequenceDurationDays') ~ '^[0-9]+$'
          AND lss.started_at < NOW() - ((t.outbound_sequence_json->>'maxSequenceDurationDays')::int * INTERVAL '1 day')
      `,
      [ck]
    );
  }
  ...
  const sequenceStaleActiveN = rows.sequenceStaleActive?.rows?.[0]?.n ?? 0;
  const sequenceDupActiveQueueN = rows.sequenceDupActiveQueue?.rows?.[0]?.n ?? 0;
  const sequenceAttemptsInStageOverMaxN = rows.sequenceAttemptsInStageOverMax?.rows?.[0]?.n ?? 0;
  const sequenceAttemptsTotalOverCapN = rows.sequenceAttemptsTotalOverCap?.rows?.[0]?.n ?? 0;
  const sequenceCalendarDurationExceededN = rows.sequenceCalendarDurationExceeded?.rows?.[0]?.n ?? 0;
  ...
  if (sequenceStaleActiveN > 0)
    problems.push({
      key: 'sequence_stale_active',
      intentId: 'sequence.stale-active-sequence',
      value: sequenceStaleActiveN
    });
  if (sequenceCalendarDurationExceededN > 0)
    problems.push({
      key: 'sequence_calendar_duration_exceeded',
      intentId: 'sequence.bounded-duration',
      value: sequenceCalendarDurationExceededN
    });
  if (sequenceAttemptsInStageOverMaxN > 0)
    problems.push({
      key: 'sequence_attempts_in_stage_over_max',
      intentId: 'sequence.bounded-attempts-per-stage',
      value: sequenceAttemptsInStageOverMaxN
    });
  if (sequenceAttemptsTotalOverCapN > 0)
    problems.push({
      key: 'sequence_attempts_total_over_cap',
      intentId: 'sequence.bounded-total-dials-per-lead',
      value: sequenceAttemptsTotalOverCapN
    });
  if (sequenceDupActiveQueueN > 0)
    problems.push({
      key: 'sequence_duplicate_active_queue',
      intentId: 'sequence.no-parallel-stages-per-lead',
      value: sequenceDupActiveQueueN
    });
  ...
  return {
    ...
    pastScheduledForN,
    sequenceStaleActiveN,
    sequenceDupActiveQueueN,
    sequenceAttemptsInStageOverMaxN,
    sequenceAttemptsTotalOverCapN,
    sequenceCalendarDurationExceededN,
    problems
  };
}
  ...
  for (const c of checks) {
    ...
    if (!payload.ok) {
      ...
      if (emailEnabled) {
        ...
        await routeAlert({
          severity: 'warning',
          title: `Ops invariant violated (${subjectCk})`,
          message: JSON.stringify(compact, null, 2),
          dedupeKey: `ops-invariant:${subjectCk}`,
          source: 'ops-invariants',
          metadata: compact,
          throttleMinutes: 60,
        }).catch(() => {});
      }
...
export function summarizeOpsInvariants(payload) {
  ...
  const labels = [
    ...
    {
      intentId: 'sequence.stale-active-sequence',
      label: 'No active lead_sequence_state rows stale without updates for 7+ days',
      problemKey: 'sequence_stale_active'
    },
    {
      intentId: 'sequence.bounded-duration',
      label: 'No active sequences exceeding tenant maxSequenceDurationDays from started_at',
      problemKey: 'sequence_calendar_duration_exceeded'
    },
    {
      intentId: 'sequence.bounded-attempts-per-stage',
      label: 'No active sequences with attempts_in_stage above the stage maxAttemptsInStage',
      problemKey: 'sequence_attempts_in_stage_over_max'
    },
    {
      intentId: 'sequence.bounded-total-dials-per-lead',
      label: 'No active sequences with attempts_total above tenant maxTotalDialsPerLead',
      problemKey: 'sequence_attempts_total_over_cap'
    },
    {
      intentId: 'sequence.no-parallel-stages-per-lead',
      label: 'No duplicate active call_queue rows for same tenant+phone',
      problemKey: 'sequence_duplicate_active_queue'
    }
  ];

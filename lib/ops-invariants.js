...
export async function checkOpsInvariants({ clientKey = null } = {}) {
  ...
  try {
    const { shouldUseDbSlotLeases, countActiveDbLeases, getMaxConcurrentSlots } = await import('./vapi-slot-lease.js');
    if (shouldUseDbSlotLeases()) {
      const n = await countActiveDbLeases();
      ...
      if (n > maxSlots) {
        results.push({
          ...
          processingStuckMinutes,
          onHourSpikeThreshold,
          retryDueThreshold: 50,
          dialBurstThreshold: 15,
          maxRetriesPerLead: 3,
          phantomN: 0,
          stuckN: 0,
          maxPerExactHour: 0,
          ...
          unknownReleaseCount: 0,
          activeDbLeases: n,
          maxDbLeases: maxSlots,
          problems: [
            {
              key: 'vapi_slot_lease_over_cap',
              intentId: 'queue.cross-instance-concurrency-cap',
              value: n,
              threshold: maxSlots
            }
          ]
    ...
    /* ignore — table missing or db not ready */
  }
...
export function summarizeOpsInvariants(payload) {
  ...
  const labels = [
    ...
    {
      intentId: 'queue.cross-instance-concurrency-cap',
      label: 'Global Vapi slot leases never exceed VAPI_MAX_CONCURRENT',
      problemKey: 'vapi_slot_lease_over_cap'
    }
  ];

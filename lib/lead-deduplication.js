...
const optOutCacheByClient = new Map(); // clientKey -> { set, loadedAt }
...
export async function checkDuplicate(clientKey, phone, options = {}) {
  ...
  try {
    ...
    const keyClause = mk ? 'phone_match_key = $2' : 'phone = $2';
    const keyVal = mk || phone;
    if (checkAllTime) {
      sql = `
        SELECT 
          id, 
 ...
 */
export async function isOptedOut(clientKey, phone) {
  try {
    const ck = String(clientKey || '').trim() || '__unknown__';
    const now = Date.now();
    const entry = optOutCacheByClient.get(ck);
    if (!entry || (now - entry.loadedAt) > OPT_OUT_CACHE_TTL) {
      await loadOptOutCache(ck);
    }
    const fresh = optOutCacheByClient.get(ck);
    return (fresh?.set || new Set()).has(phone);
 ...
 */
async function loadOptOutCache(clientKey) {
  try {
    const ck = String(clientKey || '').trim() || '__unknown__';
    const result = await query(
      ...
      SELECT phone
      FROM opt_out_list
      WHERE ${process.env.DB_TYPE === 'postgres' ? 'active = TRUE' : 'active = 1'}
        AND (client_key = $1 OR client_key = '__global__')
    `,
      [ck]
    );
    const set = new Set(result.rows.map((r) => r.phone));
    optOutCacheByClient.set(ck, { set, loadedAt: Date.now() });
    console.log(`[OPT-OUT] Loaded ${set.size} opted-out numbers for ${ck}`);
 ...
 */
export async function addToOptOut(clientKey, phone, reason = 'user_request') {
  try {
    await query(`
      INSERT INTO opt_out_list (client_key, phone, reason, opted_out_at, active, updated_at)
      VALUES ($1, $2, $3, ${process.env.DB_TYPE === 'postgres' ? 'NOW()' : "datetime('now')"}, ${process.env.DB_TYPE === 'postgres' ? 'TRUE' : 1}, ${process.env.DB_TYPE === 'postgres' ? 'NOW()' : "datetime('now')"})
      ON CONFLICT (client_key, phone)
      DO UPDATE SET active = ${process.env.DB_TYPE === 'postgres' ? 'TRUE' : 1}, opted_out_at = ${process.env.DB_TYPE === 'postgres' ? 'NOW()' : "datetime('now')"}, reason = $3, updated_at = ${process.env.DB_TYPE === 'postgres' ? 'NOW()' : "datetime('now')"}
    `, [String(clientKey || '').trim() || '__unknown__', phone, reason]);
    
    // Update cache
    const ck = String(clientKey || '').trim() || '__unknown__';
    const entry = optOutCacheByClient.get(ck);
    if (entry?.set) entry.set.add(phone);
 ...
 * @returns {Promise<Object>} - Result
 */
export async function removeFromOptOut(clientKey, phone) {
  try {
    await query(`
      UPDATE opt_out_list 
      SET active = ${process.env.DB_TYPE === 'postgres' ? 'FALSE' : 0}, updated_at = ${process.env.DB_TYPE === 'postgres' ? 'NOW()' : "datetime('now')"}
      WHERE client_key = $1 AND phone = $2
    `, [String(clientKey || '').trim() || '__unknown__', phone]);
    
    // Update cache
    const ck = String(clientKey || '').trim() || '__unknown__';
    const entry = optOutCacheByClient.get(ck);
    if (entry?.set) entry.set.delete(phone);
...
export async function processLeadForImport(lead, clientKey) {
  ...
  const optedOut = await isOptedOut(clientKey, normalizedPhone);
  if (optedOut) {
...
/** @deprecated Use bulkProcessLeads — alias fixes legacy imports */
export const processBulkLeads = bulkProcessLeads;

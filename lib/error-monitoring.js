...
export async function sendCriticalAlert(alertData) {
  ...
  // Suppress email/Slack when alerts are disabled (e.g. maintenance or low-activity)
  if (process.env.ALERTS_SUPPRESSED === 'true' || process.env.MAINTENANCE_MODE === 'true') {
    console.log('[CRITICAL ALERT] (suppressed – ALERTS_SUPPRESSED or MAINTENANCE_MODE set)');
    return;
  }
...
export async function getErrorStats(options = {}) {
  ...
  try {
    const filters = [`logged_at >= NOW() - INTERVAL '${days} days'`];
    const params = [];
    let paramCount = 1;

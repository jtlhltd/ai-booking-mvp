...
/**
 * Send critical/error alert via severity-tiered router (grouped incidents).
 */
export async function sendCriticalAlert(alertData) {
  const { message, errorType, count, severity, metadata } = alertData;
  console.error('[CRITICAL ALERT]', message);
  const { routeAlert } = await import('./alert-router.js');
  return routeAlert({
    severity: severity || 'critical',
    title: errorType || 'System Error',
    message: `${message}${count ? `\nCount: ${count}` : ''}`,
    dedupeKey: `error:${errorType || 'system'}`,
    source: 'error-monitoring',
    metadata: { ...(metadata || {}), count, errorType },
  });
}

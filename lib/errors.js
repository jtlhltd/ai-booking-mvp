...
/**
 * Error logging utility with structured logging integration
 */
export function logError(error, req = null, additionalContext = {}) {
  ...
  // Try to use structured logger if available (async, fire and forget)
  import('./structured-logger.js').then(({ getLogger }) => {
    const logger = getLogger({ component: 'error-handler' });
    
    if (error.statusCode >= 500) {
      logger.error(error.message, error, logData.context);
    } else if (error.statusCode >= 400) {
      logger.warn(error.message, error, logData.context);
    } else {
      logger.info(error.message, logData.context);
    }
  }).catch(() => {
    // Fallback to console logging if structured logger unavailable
    if (error.statusCode >= 500) {
  ...
  // Also track in performance monitor if available (async, fire and forget)
  import('./performance-monitor.js').then(({ getPerformanceMonitor }) => {
    const monitor = getPerformanceMonitor();
    monitor.trackError(error, logData.context);
  }).catch(() => {
    // Silently fail if performance monitor unavailable
  });

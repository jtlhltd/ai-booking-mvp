...
export async function cloneVapiAssistant(templateAssistantId, clientData) {
  ...
  try {
    ...
    if (!templateResponse.ok) {
      ...
      let errorMessage = `Failed to get template assistant: ${templateResponse.status}`;
      try {
    ...
    // Create new assistant - remove properties that shouldn't be sent
    const { id, orgId, createdAt, updatedAt, isServerUrlSecretSet, tools, ...templateWithoutMeta } = template;
    
    // Get serverUrl from environment
    ...
    const newAssistant = {
      ...
      serverUrl: template.serverUrl || serverUrl, // Preserve template serverUrl or use default
      // Note: tools are not included in creation - they must be added via PATCH after creation
      model: {
        ...template.model,
        provider: template.model?.provider || 'openai',
        model: template.model?.model || 'gpt-3.5-turbo',
        temperature: template.model?.temperature || 0.7,
        maxTokens: template.model?.maxTokens || 250,
        messages: [
          {
            role: 'system',
    ...
    // Add tools via PATCH if template had tools
    if (tools && Array.isArray(tools) && tools.length > 0) {
      try {
        const patchResponse = await fetch(`https://api.vapi.ai/assistant/${created.id}`, {
          method: 'PATCH',
          headers: {
            'Authorization': `Bearer ${VAPI_PRIVATE_KEY}`,
            'Content-Type': 'application/json'
        ...
        if (patchResponse.ok) {
          console.log(`[ONBOARDING] Added ${tools.length} tool(s) to assistant`);
        } else {
          console.warn(`[ONBOARDING] Could not add tools: ${patchResponse.status}`);
        }
      } catch (error) {
        console.warn(`[ONBOARDING] Error adding tools: ${error.message}`);
      }
    }
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <div style="text-align: center; padding: 20px; color: #999; font-size: 12px;">
        <p>AI Booking System - Automated Lead Conversion</p>
      </div>
    </div>
  `;
 ...
 */
function mergeWhiteLabel(base = {}, patch = {}) {
  const out = { ...base, ...patch };
  if (base.pricing || patch.pricing) {
    out.pricing = { ...(base.pricing || {}), ...(patch.pricing || {}) };
  }
  if (base.branding || patch.branding) {
    out.branding = { ...(base.branding || {}), ...(patch.branding || {}) };
  }
  return out;
}
...
export async function updateClientConfig(clientKey, updates) {
  try {
    ...
    let whiteLabel = mergeWhiteLabel(client.whiteLabel || {}, whiteLabelPatch || {});
    if (monthlyFee !== undefined && monthlyFee !== null && monthlyFee !== '') {
      const n = Number(monthlyFee);
      if (Number.isFinite(n) && n >= 0) whiteLabel.monthlyFee = n;
    }
    if (avgDealValue !== undefined && avgDealValue !== null && avgDealValue !== '') {
      const n = Number(avgDealValue);
      if (Number.isFinite(n) && n >= 0) whiteLabel.avgDealValue = n;
    }
    if (pricing && typeof pricing === 'object') {
      whiteLabel.pricing = { ...(whiteLabel.pricing || {}), ...pricing };
    }

    if (rest.clientKey != null) delete rest.clientKey;

    const vapiMerged =
      vapiPatch && typeof vapiPatch === 'object'
        ? { ...(client.vapi || {}), ...vapiPatch }
        : client.vapi || {};
    if (vapiPatch && typeof vapiPatch === 'object') {
      const pruneIfBlank = [
      ...
      for (const k of pruneIfBlank) {
        if (Object.prototype.hasOwnProperty.call(vapiPatch, k)) {
          const v = vapiMerged[k];
          if (v === '' || v == null) delete vapiMerged[k];
        }
      }
    }
    ...
    if (vapiPatch && typeof vapiPatch === 'object') {
      if (vapiPatch.assistantId != null && String(vapiPatch.assistantId).trim()) {
        updatedClient.vapiAssistantId = String(vapiPatch.assistantId).trim();
      }
      if (vapiPatch.phoneNumberId != null && String(vapiPatch.phoneNumberId).trim()) {
        updatedClient.vapiPhoneNumberId = String(vapiPatch.phoneNumberId).trim();
      }
    }
    ...
    const saved = await getFullClient(clientKey, { bypassCache: true });
    return { ok: true, success: true, client: saved };

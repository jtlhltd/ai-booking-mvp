...
import { startSpan, startSpanSync } from './sentry.js';
...
export function parseCSV(csvData, mapping = {}) {
  return startSpanSync(
    { name: 'lead.import.csv_parse', op: 'lead.import' },
    () => parseCSVInner(csvData, mapping)
  );
}

function parseCSVInner(csvData, mapping = {}) {
...
export async function importLeads(clientKey, leads, options = {}) {
  return startSpan(
    { name: 'lead.import.batch_insert', op: 'lead.import', attributes: { clientKey } },
    async () => importLeadsInner(clientKey, leads, options)
  );
}

async function importLeadsInner(clientKey, leads, options = {}) {
  const {

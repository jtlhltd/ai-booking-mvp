...
export function registerScheduledJobs(deps = {}) {
  ...
  // Call queue even minutes; retry queue odd minutes — avoids hammering Postgres with concurrent
  // call_queue scans. Main processor uses getPendingCalls (vapi_call only); request-queue handles sms_send/lead_import only.
  ...
  console.log('✅ Call queue + request-queue processor scheduled (even minutes UTC)');
  ...
  console.log('✅ Retry queue processor scheduled (odd minutes UTC)');

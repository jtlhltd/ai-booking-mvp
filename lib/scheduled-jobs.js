...
function wrapCron(cronName, fn, { monitorSlug, cronExpr } = {}) {
  return async () => {
    const run = () => startSpan({ name: cronName, op: 'cron', attributes: { cronName } }, fn);
    if (isSentryEnabled() && monitorSlug && cronExpr) {
      await Sentry.withMonitor(monitorSlug, run, {
...
export function registerScheduledJobs(deps = {}) {
  ...
  }, { monitorSlug: 'call-queue-processor', cronExpr: callQueueCronExpr }));
  ...
  }, { monitorSlug: 'retry-and-request-queue', cronExpr: retryQueueCronExpr }));
  ...
  }, { monitorSlug: 'stuck-processing-reaper', cronExpr: stuckReaperCronExpr }));
  ...
  }, { monitorSlug: 'new-lead-queuer', cronExpr: newLeadQueuerCronExpr }));
  ...
  importFn('./webhook-retry.js').then(({ processWebhookRetryQueue }) => {
    ...
    }, { monitorSlug: 'webhook-retry-processor', cronExpr: webhookRetryCronExpr }));

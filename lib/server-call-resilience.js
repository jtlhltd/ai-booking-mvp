...
export function createVapiFailureHandlers(deps) {
  ...
  async function scheduleRetryCall({ from, tenantKey, client, error, rule = null }) {
    try {
      const errorType = categorizeError({ message: error });
      const delayMinutes =
        rule?.retryDelayMinutes != null
          ? Number(rule.retryDelayMinutes)
          : errorType === 'rate_limit'
            ...
            : errorType === 'server_error'
              ...
              : errorType === 'critical'
                ...
                : 30;
      const maxRetries =
        rule?.maxRetries != null ? Number(rule.maxRetries) : 3;
      const retryDelay = Math.max(1, delayMinutes) * 60 * 1000;
  ...
  async function handleVapiFailure({ from, tenantKey, error }) {
    try {
      ...
      const errorType = categorizeError({ message: error });
      const { executeRemediation, matchRemediationRule } = await import('./remediation.js');
      const rule = matchRemediationRule('vapi_call_failure', errorType);
      await executeRemediation('vapi_call_failure', errorType, {
        from,
        tenantKey,
        client,
        error,
        rule,
        handlers: {
          sms_fallback: (ctx) => sendSmsFallback(ctx),
          schedule_retry: (ctx) =>
            scheduleRetryCall({ ...ctx, rule: ctx.rule || rule }),
          update_lead_status: (ctx) => updateLeadOnVapiFailure(ctx),
        },
      });
    } catch (fallbackError) {

...
import { dbType, pool, query } from '../db.js';
...
const pendingQueryPerformanceMetrics = new Map();
let queryPerformanceFlushScheduled = false;
...
function enqueueQueryPerformanceMetric({ queryHash, queryPreview, duration }) {
  const existing = pendingQueryPerformanceMetrics.get(queryHash);
  if (existing) {
    existing.avgDuration =
      (existing.avgDuration * existing.callCount + duration) / (existing.callCount + 1);
    existing.maxDuration = Math.max(existing.maxDuration, duration);
    existing.callCount += 1;
  } else {
    pendingQueryPerformanceMetrics.set(queryHash, {
      queryHash,
      queryPreview,
  ...
  if (!queryPerformanceFlushScheduled) {
    queryPerformanceFlushScheduled = true;
    setImmediate(() => {
      flushQueryPerformanceMetrics().catch(() => {});
    });
  }
}

async function executeQueryPerformanceWrite(sql, params) {
  const isPg = String(dbType || '').startsWith('post');
  if (isPg && pool && typeof pool.query === 'function') {
    return await pool.query(sql, params);
  }
  return await query(sql, params);
}

async function flushQueryPerformanceMetrics() {
  queryPerformanceFlushScheduled = false;
  const metrics = Array.from(pendingQueryPerformanceMetrics.values());
  pendingQueryPerformanceMetrics.clear();
  if (metrics.length === 0) {
    return;
  }

  const params = [];
  const valuesSql = metrics.map((metric, idx) => {
    const base = idx * 5;
    params.push(
      metric.queryHash,
      metric.queryPreview,
  ...
  // Use the raw PG pool for tracker writes so db.js does not track its own metrics update.
  await executeQueryPerformanceWrite(`
    INSERT INTO query_performance (query_hash, query_preview, avg_duration, max_duration, call_count, last_executed_at)
    VALUES ${valuesSql}
    ON CONFLICT (query_hash)
    DO UPDATE SET
      avg_duration = (
        query_performance.avg_duration * query_performance.call_count::numeric
        + EXCLUDED.avg_duration * EXCLUDED.call_count
      ) / (query_performance.call_count + EXCLUDED.call_count),
      max_duration = GREATEST(query_performance.max_duration, EXCLUDED.max_duration),
      call_count = query_performance.call_count + EXCLUDED.call_count,
...
export async function trackQueryPerformance(queryText, duration, params = []) {
  try {
    ...
    // call_queue / retry_queue stale-processing self-heal (cron every 2m): indexed CTE + bulk reset.
    const isQueueStaleSelfHeal =
      collapsed.includes('with stale as') &&
      collapsed.includes("status = 'processing'") &&
      collapsed.includes("interval '10 minutes'") &&
      (collapsed.includes('update call_queue') || collapsed.includes('update retry_queue'));
    ...
    // getFullClient / listFullClients hot path (large JSON row); slow = pool wait under cron load.
    const isTenantFullConfigRead =
      collapsed.startsWith('select') &&
      collapsed.includes('from tenants') &&
      collapsed.includes('outbound_sequence_json') &&
      collapsed.includes('is_enabled');

    // Ops invariants top-of-hour smear + spike check (cron ~5m): bounds CTE over call_queue.
    const isOpsInvariantsCallQueueBoundsQuery =
      collapsed.includes('with bounds as') &&
      collapsed.includes('cross join bounds') &&
      collapsed.includes('from call_queue cq') &&
      collapsed.includes('extract(minute from (cq.scheduled_for at time zone');

    // Hourly rate_limit_tracking retention cleanup (middleware/security.js setInterval).
    const isRateLimitTrackingCleanup =
      collapsed.includes('delete from rate_limit_tracking') &&
      collapsed.includes('window_start <');

    // getRemindersDue / getFollowUpsDue (cron ~5m): WITH picked + retry_reason LIKE prefix on retry_queue.
    const isRetryQueueReasonDuePoll =
      collapsed.includes('with picked as') &&
      collapsed.includes('from retry_queue') &&
      collapsed.includes('retry_reason like') &&
      collapsed.includes("status = 'pending'") &&
      collapsed.includes('scheduled_for <=') &&
      collapsed.includes('escape');

    // Webhook retry cron poll (every 5m): indexed pending + retry_type LIKE 'webhook_%'.
    const isWebhookRetryQueuePoll =
      collapsed.includes('from retry_queue') &&
      collapsed.includes("retry_type like 'webhook_%'") &&
      collapsed.includes("status = 'pending'") &&
      collapsed.includes('scheduled_for <=') &&
      collapsed.includes('retry_attempt <=');

    // Reminder cron poll (every 5m): trivial indexed read; slow = pool/queue wait under load.
    const isAppointmentRemindersPendingPoll =
      collapsed.includes('from appointment_reminders') &&
      collapsed.includes('status = ') &&
      collapsed.includes('pending') &&
      collapsed.includes('scheduled_for <=') &&
      collapsed.includes('order by scheduled_for');

    if (isAppointmentRemindersPendingPoll) {
      if (duration >= CRITICAL_QUERY_THRESHOLD) {
        console.warn(
          `[SLOW QUERY] ${duration}ms (not tracked — appointment_reminders pending poll; duration is usually pool/queue wait, not SQL cost)`
        );
      }
      return null;
    }

    if (isTenantFullConfigRead) {
      if (duration >= CRITICAL_QUERY_THRESHOLD) {
        console.warn(
          `[SLOW QUERY] ${duration}ms (not tracked — tenants full config read; duration is usually pool/queue wait, not SQL cost)`
        );
      }
      return null;
    }

    if (isOpsInvariantsCallQueueBoundsQuery) {
      if (duration >= CRITICAL_QUERY_THRESHOLD) {
        console.warn(
          `[SLOW QUERY] ${duration}ms (not tracked — ops invariants call_queue bounds check; duration is usually pool/queue wait, not SQL cost)`
        );
      }
      return null;
    }

    if (isWebhookRetryQueuePoll) {
      if (duration >= CRITICAL_QUERY_THRESHOLD) {
        console.warn(
          `[SLOW QUERY] ${duration}ms (not tracked — webhook retry_queue due poll; duration is usually pool/queue wait, not SQL cost)`
        );
      }
      return null;
    }

    if (isRetryQueueReasonDuePoll) {
      if (duration >= CRITICAL_QUERY_THRESHOLD) {
        console.warn(
          `[SLOW QUERY] ${duration}ms (not tracked — retry_queue reason-prefix due poll; duration is usually pool/queue wait, not SQL cost)`
        );
      }
      return null;
    }

    if (isRateLimitTrackingCleanup) {
      if (duration >= CRITICAL_QUERY_THRESHOLD) {
        console.warn(
          `[SLOW QUERY] ${duration}ms (not tracked — rate_limit_tracking retention cleanup; duration is usually pool/queue wait, not SQL cost)`
        );
      }
      return null;
    }

    if (isQueueStaleSelfHeal) {
      if (duration >= CRITICAL_QUERY_THRESHOLD) {
        console.warn(
          `[SLOW QUERY] ${duration}ms (not tracked — queue stale-processing self-heal; duration is usually pool/queue wait, not SQL cost)`
        );
      }
      return null;
    }
    ...
    enqueueQueryPerformanceMetric({ queryHash, queryPreview, duration });

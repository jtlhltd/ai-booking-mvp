...
/** Min ms between critical emails for the same query shape (avoids hundreds when DB is saturated). */
const SLOW_QUERY_EMAIL_COOLDOWN_MS = parseInt(process.env.SLOW_QUERY_EMAIL_COOLDOWN_MS, 10) || 15 * 60 * 1000;
/** Max critical slow-query emails in a sliding window across all shapes. */
const SLOW_QUERY_EMAIL_GLOBAL_MAX = parseInt(process.env.SLOW_QUERY_EMAIL_GLOBAL_MAX, 10) || 12;
const SLOW_QUERY_EMAIL_GLOBAL_WINDOW_MS =
  parseInt(process.env.SLOW_QUERY_EMAIL_GLOBAL_WINDOW_MS, 10) || 30 * 60 * 1000;
...
export async function trackQueryPerformance(queryText, duration, params = []) {
  try {
    ...
    if (
      normalizedQuery.startsWith('select 1') ||
      normalizedQuery.includes('select 1 as healthy') ||
      (normalizedQuery.includes('as healthy') && normalizedQuery.includes('now()'))
    ) {
      return null;
    }
    ...
    const isCallQueuePkTouch =
      normalizedQuery.includes('update call_queue') && /\bwhere\s+id\s*=/.test(normalizedQuery);

    // Check if this is a slow query
    const isSlow = duration >= SLOW_QUERY_THRESHOLD;
    const pastCriticalThreshold = duration >= CRITICAL_QUERY_THRESHOLD;
    const isCritical = pastCriticalThreshold && !isCallQueuePkTouch;
    ...
    if (isCritical) {
      ...
      const now = Date.now();
      const emailDecision = shouldSendCriticalSlowQueryEmail(queryHash, now);
      if (emailDecision.send) {
        recordCriticalSlowQueryEmail(queryHash, now);
        sendCriticalAlert({
        ...
        console.warn(
          `[QUERY TRACKER] Critical slow query email suppressed (${emailDecision.reason}); check logs / DB metrics. ${duration}ms hash=${queryHash}`
        );
      }
    }

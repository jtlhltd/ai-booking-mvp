// tests/admin/test-admin-business-stats.js
// Test admin business stats endpoint
...
describe('Admin Business Stats Tests', () => {
  ...
  test('Get business stats', async () => {
    skipIf(!await checkServerAvailable(BASE_URL), 'Server not available');
    
    const response = await httpRequest(`${BASE_URL}/api/admin/business-stats`, {
      method: 'GET',
      headers: {
        'X-API-Key': API_KEY
      }
    ...
    assertTrue(response.ok || response.status === 200, 'Business stats retrieved');
    if (response.data) {
      assertTrue(typeof response.data === 'object', 'Stats data returned');
    }
  });
});

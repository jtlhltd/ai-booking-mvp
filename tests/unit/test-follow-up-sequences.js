...
describe('Follow-up Sequences Tests', () => {
  ...
  test('Sequence selection - no-answer', () => {
    ...
    assertTrue(sequence.scheduleMode === 'same_week_weekdays', 'Uses weekday pass mode');
  ...
  test('Sequence selection - not-interested', () => {
    ...
    assertTrue(sequence.steps.length === 0, 'No automated calls for not interested');
  ...
  test('Sequence selection - callback-requested', () => {
    ...
    assertTrue(sequence.steps.length === 0, 'No default steps for callback-requested');
  });
  ...
  test('Sequence step structure (delay-based — interested path)', () => {
    const sequence = getFollowUpSequence('interested');
  ...
  test('Delay calculation (interested_no_booking)', () => {
    const sequence = getFollowUpSequence('interested');
    if (sequence && sequence.steps.length > 0) {
  ...
  test('Sequence step channels (interested)', () => {
    const sequence = getFollowUpSequence('interested');
    if (sequence && sequence.steps.length > 0) {
  ...
  test('Sequence step count', () => {
    const withSteps = getFollowUpSequence('interested');
    assertTrue(withSteps && withSteps.steps.length > 0, 'Interested sequence has delay-based steps');
    const noAnswer = getFollowUpSequence('no-answer');
    assertTrue(noAnswer && noAnswer.scheduleMode === 'same_week_weekdays', 'No-answer uses computed weekday slots');
  });
});

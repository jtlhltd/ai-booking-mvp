...
Write-Host "=== Sentry auto-trigger (no manual alert needed) ==="
Write-Host "SENTRY_SELF_HEAL_POLLER_ENABLED=true on Render polls Sentry every 5 min and fires the relay."
Write-Host "SENTRY_AUTH_TOKEN (single token: Issue&Event R+W, Alerts R+W, Org Read, Project Read)"
Write-Host "  https://jtlh-ltd.sentry.io/settings/account/api/auth-tokens/"
Write-Host "  Put in .cursor/self-heal-secrets.env then: .\scripts\sync-self-heal-env-to-render.ps1"
Write-Host ""
Write-Host "=== Optional Sentry alert (UI backup) ==="

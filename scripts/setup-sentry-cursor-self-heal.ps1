# Wire Sentry issue alerts → Cursor self-heal automation (relay on Render).
...
Write-Host "Sentry → Cursor self-heal setup"
...
Write-Host "=== Sentry alert (2 min, one-time) ==="
Write-Host "1. Open https://$Organization.sentry.io/alerts/$Project/new/issue/"
Write-Host "2. Name: Cursor self-heal relay"
Write-Host "3. When: A new issue is created OR The issue changes state from resolved to unresolved"
...
Write-Host "6. Save & enable"
Write-Host ""

Write-Host "=== Cursor automation prompt (must Save in editor) ==="
Write-Host "- Triggers: Sentry Any issue event + Webhook"
Write-Host "- After opening PR: gh pr ready <n> then merge for GREEN tier"

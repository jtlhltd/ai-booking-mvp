...
| `ops.automation-smoke-no-throw` | `GET /automation-smoke` is a deployment verification endpoint, not the failure injector. When `AUTOMATION_SMOKE_ENABLED=true` and Sentry is configured, it must return `{ ok: true, message: <string> }` without throwing; `/heal-test` remains the armed self-heal failure path. | `lib/automation-smoke-probe.js`, `routes/health-probes-mount.js` | canary (`tests/routes/health-probes-mount.contract.test.js`) | With `AUTOMATION_SMOKE_ENABLED=true`, hit `/automation-smoke`; expect HTTP 200 JSON and no new Sentry error for the request. |

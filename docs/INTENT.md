...
| `queue.no-phantom-completed` | Rows MUST NOT transition to `status='completed'` with `initiated_call_id IS NULL`. That indicates a worker bug or race. | `lib/instant-calling.js`, queue worker write paths, Postgres **call_queue** constraint, SQLite **call_queue** CHECK and migrateSqliteCallQueuePhantomConstraint in db.js | invariant, canary | **lib/ops-invariants** phantom query returns 0 on Postgres. On SQLite, INSERT of a completed vapi_call row without initiated_call_id fails after migration; see canary sqlite-call-queue-phantom-check under tests/canaries. |
...
| `queue.concurrency-cap` | The queue worker must never have more than `VAPI_MAX_CONCURRENT` (default 1) Vapi requests in flight at once. With DB slot leases enabled (Postgres/SQLite, default outside Jest), `lib/vapi-slot-lease.js` coordinates the cap across processes; in Jest or when `VAPI_DB_SLOT_LEASE=0`, `acquireVapiSlot` uses the in-process limiter only. | `lib/instant-calling.js#acquireVapiSlot`, `lib/vapi-slot-lease.js` | canary | Fire `N+1` simultaneous `acquireVapiSlot()` calls; the surplus must remain pending until a release. |
| `queue.cross-instance-concurrency-cap` | With DB slot leases enabled, `SELECT COUNT(*) FROM vapi_slot_leases WHERE expires_at > now()` (Postgres) or the SQLite equivalent must never exceed `VAPI_MAX_CONCURRENT`. | `lib/vapi-slot-lease.js`, `lib/instant-calling.js`, `lib/ops-invariants.js` | canary, invariant | `lib/ops-invariants.js` must not emit `vapi_slot_lease_over_cap`. Two instance ids against one DB cannot insert more than `max` non-expired rows (see `tests/canaries/cross-instance-concurrency-cap.canary.test.js`). |
...
| `tenant.scoped-reads-require-api-key` | Tenant-scoped JSON surfaces (`GET/POST /api/dnc/*`, `GET /api/ops/health/:clientKey`, quick-win metrics under `/api/sms-delivery-rate|calendar-sync|quality-metrics`) must return **401** when `X-API-Key` is missing (before body reveals tenant existence). With a valid tenant API key, `clientKey` in the path/query/body must match the key’s tenant unless the key has admin permissions. | `routes/ops-health-and-dnc.js`, `routes/quick-win-metrics.js` | policy, canary | `curl` any listed route without `X-API-Key` → 401. With tenant A’s key, request tenant B’s `clientKey` → 403. |
| `tenant.daily-summary-public-readonly` | `GET /api/daily-summary/:clientKey` is a public, read-only dashboard surface. It must be `Cache-Control: no-store` and must return a safe `{ ok: true, configured: false, ... }` payload for unknown/unconfigured tenants (avoid 401/403/500 that leaks tenant existence). | `routes/daily-summary.js` | canary | `curl /api/daily-summary/not-a-tenant` returns 200 JSON with `ok: true` and `configured: false`, and `Cache-Control` includes `no-store`. |

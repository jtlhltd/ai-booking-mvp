...
| `queue.worker-imports-scheduling-and-resilience` | `lib/server-queue-workers.js` MUST statically import `selectOptimalAssistant` from `./server-assistant-scheduling.js` and `categorizeError` from `./server-call-resilience.js` whenever those symbols are used, so queue/retry cron paths never throw `ReferenceError` at runtime. | `lib/server-queue-workers.js` | policy | Remove either `import` locally; `npm run check:policy` must fail. |

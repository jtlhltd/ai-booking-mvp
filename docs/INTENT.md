...
| `dial.lead-dial-context-contained` | Optional per-lead `leads.lead_dial_context_json` is for dial-time variable overlay only. Only allow-listed modules may reference the column name; `parseLeadDialContextFromDb` must strip reserved keys (`leadName`, `tenantBusinessName`, `decisionMakerName`, `decisionMakerRole`, `bestCallbackWindow`, `priorCallWasSubstantive`, `daysSinceLastCall`, `isFollowUpCall`, `isFinalStage`, `client_key`, `tenant_key`, `phone`, `lead_contact_phone`, `email`) so sequence/worker-owned `variableValues` cannot be overridden by imports; writers must enforce `LEAD_DIAL_CONTEXT_MAX_BYTES`. | `db/migrations/`, `lib/lead-dial-context.js`, `db.js` (when mapped), `lib/instant-calling.js` / `lib/server-queue-workers.js` (when merged into payload) | policy, canary | Store JSON with a reserved key; after parse for dial overlay, that key is absent while a safe custom key remains. |

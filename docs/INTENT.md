...
| `consumer.no-logistics-sheet-when-disabled` | Core logistics Google Sheet writes default **off**. When `LOGISTICS_SHEET_WRITES_IN_CORE` is not `1`, core MUST NOT append/update logistics sheets on EOCR, MUST skip legacy logistics tool-call handlers, and MUST skip `sheet_patch` retry processing. Consumer webhooks and dial queue unchanged. | `lib/vapi-webhooks/process-webhook-payload.js`, `lib/server-queue-workers.js#processSheetPatchRetry`, `lib/logistics-sheet-writes-in-core.js` | canary, unit | Unset env or `LOGISTICS_SHEET_WRITES_IN_CORE=0`; EOCR must not call `appendLogistics`; `processSheetPatchRetry` returns without sheet I/O. |
| `consumer.vertical-routes-not-on-core` | Call Bot MUST NOT mount Tom logistics CRM routers (`follow-up-queue`, `daily-summary`, `/tools/*`, `admin/vapi/logistics-*`). Consumer apps own that surface. | `app/mount-api.js`, `app/mount-admin-tools.js` | policy, unit | `app/mount-api.js` has no `createFollowUpQueueRouter` / `createDailySummaryRouter`; `mount-admin-tools.js` has no `createToolsRouter` / `createAdminVapiLogisticsRouter`. |

...
| `ops.self-heal-probes-healthy-when-armed` | `GET /automation-smoke` and `GET /heal-test` are GREEN-tier self-heal verification arms. When their matching enable flag is true and Sentry is configured, each must return a 2xx JSON success envelope instead of intentionally throwing. | `lib/automation-smoke-probe.js`, `lib/heal-test-probe.js`, `routes/health-probes-mount.js` | route contract (`tests/routes/health-probes-mount.contract.test.js`) | With `AUTOMATION_SMOKE_ENABLED=true` and Sentry configured, request `/automation-smoke`; response should be `{ ok: true, message: 'automation smoke probe healthy' }`, not a 500. With `HEAL_TEST_ENABLED=true` and Sentry configured, request `/heal-test`; response should be `{ ok: true, message: 'heal test probe healthy' }`, not a 500. |

...
| `dial.lead-dial-context-contained` | Optional per-lead `leads.lead_dial_context_json` is a tightly contained source for sanitized outbound context. Only allow-listed modules may reference the column name; `normalizeLeadDialContext` must strip reserved keys (`leadName`, `tenantBusinessName`, `decisionMakerName`, `decisionMakerRole`, `bestCallbackWindow`, `priorCallWasSubstantive`, `daysSinceLastCall`, `isFollowUpCall`, `isFinalStage`, `client_key`, `tenant_key`, `phone`, `lead_contact_phone`, `email`), keep scalar values only, and drop oversize payloads. In `callLeadInstantly`, the sanitized lead context is applied as the final shallow `assistantOverrides.variableValues` overlay after sequence and A/B merges; in the sequence webhook it may only feed the separate allow-listed `qual._importContext` handoff path. | `db/migrations/`, `lib/lead-dial-context.js`, `lib/server-queue-workers.js`, `lib/instant-calling.js`, `lib/vapi-webhooks/outbound-sequence-webhook.js` | policy, canary | Store JSON with a reserved key and safe custom keys; on the dial payload, the reserved key is absent and only non-reserved `variableValues` survive. On sequence handoff rows, only the configured/default `_importContext` keys may appear, never the raw blob or reserved keys. |
...
| `sequence.operator-stop-dismiss-audited` | Admin/operator stop and salvage-dismiss actions must be explicit mutations: stopping a lead cancels only future `sequence_next` queue rows for that lead and marks the sequence abandoned, while dismissing abandoned salvage stamps `qual._salvageDismissedAt` / `qual._salvageDismissedBy` so the row leaves the default abandoned cohort but remains visible in `all`. Both actions must append a bounded FIFO `qual._opsAudit` trail. | `lib/outbound-sequence-ops.js`, `routes/client-ops-mount.js`, `lib/dashboard-follow-up-filters.js`, `public/client-dashboard.html` | canary | Stop an active sequence from the operator UI/API; future `sequence_next` rows for that phone are cancelled and the sequence row becomes abandoned. Then dismiss an abandoned salvage row; it disappears from the abandoned filter, remains in `all`, and the handoff JSON shows bounded `_opsAudit` plus `_salvageDismissedAt`. |

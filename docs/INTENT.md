...
| `sequence.enrollable-leads-visibility` | Dashboard sequence window must list leads not opted into multi-call via `GET /outbound-sequence/:clientKey/enrollable-leads` (In sequence vs Not enrolled toggle). Summary includes sampled enrollment counts. Sequence-state rows expose pending `sequence_next` queue hints and stuck hints where applicable. | `routes/outbound-sequence-visibility-mount.js`, `public/client-dashboard.html` | canary | Toggle Not enrolled; only `sequenceOptedIn: false` leads appear. Summary shows enrolled/not-enrolled counts. Active row with stale `updated_at` shows stuck badge. |

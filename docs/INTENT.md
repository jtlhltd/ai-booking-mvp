...
| `dial.lead-dial-context-contained` | Optional per-lead `leads.lead_dial_context_json` is for dial-time variable overlay only. Only allow-listed modules may reference the column name; `normalizeLeadDialContext` must strip reserved keys (`leadName`, `tenantBusinessName`, `decisionMakerName`, `decisionMakerRole`, `bestCallbackWindow`, `priorCallWasSubstantive`, `daysSinceLastCall`, `isFollowUpCall`, `isFinalStage`, `client_key`, `tenant_key`, `phone`, `lead_contact_phone`, `email`), keep scalar values only, and drop oversize payloads. In `callLeadInstantly`, the sanitized lead context is applied as the final shallow `assistantOverrides.variableValues` overlay after sequence and A/B merges. | `db/migrations/`, `lib/lead-dial-context.js`, `lib/server-queue-workers.js`, `lib/instant-calling.js` | policy, canary | Store JSON with a reserved key and a safe custom key; on the dial payload, the reserved key is absent, the safe key survives, and lead context wins only on its own non-reserved `variableValues` keys. |

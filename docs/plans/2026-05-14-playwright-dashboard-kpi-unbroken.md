...
- **2026-05-14:** Load fallback no longer bails when `__dashboardInitStarted` is true (that blocked retries when init started but never painted KPIs). Defer fallback 50ms, detect dash-like glyphs, and reset dedupe once for a single retry.
- **2026-05-14:** Removed `__dashboardKpiStripPainted` (it could not distinguish static `—` from real data). `initDashboard` dedupe now **re-enters** when `#statusTotalLeads` is still the static shell so a stuck `__dashboardInitStarted` cannot permanently block paint; load fallback no longer consults the painted flag.
- **2026-05-14:** Tiny inline script immediately after the KPI `<section>` replaces static dash placeholders on the four core count tiles with `0` before the large bundle runs (slow parse no longer looks identical to a dead boot). Defined missing `OUTREACH_DASHBOARD_CLIENT_KEYS_FALLBACK` (empty; prefer server `dashboardOutreachMode`). `updateStatusBar` surfaces a banner if `#statusTotalLeads` / `#statusTotalCalls` nodes are missing.
